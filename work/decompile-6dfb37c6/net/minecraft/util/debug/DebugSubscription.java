...
public class DebugSubscription<T> {
    ...
    public DebugSubscription.Update<T> packUpdate(@Nullable T value) {
        return new DebugSubscription.Update<T>(this, Optional.ofNullable(value));
    }

    public DebugSubscription.Update<T> emptyUpdate() {
        return new DebugSubscription.Update<T>(this, Optional.empty());
    }

    public DebugSubscription.Event<T> packEvent(T value) {
        return new DebugSubscription.Event<T>(this, value);
    }
    ...
    public static record Update<T>(DebugSubscription<T> subscription, Optional<T> value) {
        ...
        public static final StreamCodec<RegistryFriendlyByteBuf, DebugSubscription.Update<?>> STREAM_CODEC = ByteBufCodecs.registry(Registries.DEBUG_SUBSCRIPTION).dispatch(DebugSubscription.Update::subscription, DebugSubscription.Update::streamCodec);
        ...
        private static <T> StreamCodec<? super RegistryFriendlyByteBuf, DebugSubscription.Update<T>> streamCodec(DebugSubscription<T> subscription) {
            return ByteBufCodecs.optional((StreamCodec) Objects.requireNonNull(subscription.valueStreamCodec)).map((optional) -> {
                return new DebugSubscription.Update(subscription, optional);
    ...
    public static record Event<T>(DebugSubscription<T> subscription, T value) {
        ...
        public static final StreamCodec<RegistryFriendlyByteBuf, DebugSubscription.Event<?>> STREAM_CODEC = ByteBufCodecs.registry(Registries.DEBUG_SUBSCRIPTION).dispatch(DebugSubscription.Event::subscription, DebugSubscription.Event::streamCodec);
        ...
        private static <T> StreamCodec<? super RegistryFriendlyByteBuf, DebugSubscription.Event<T>> streamCodec(DebugSubscription<T> subscription) {
            return ((StreamCodec) Objects.requireNonNull(subscription.valueStreamCodec)).map((object) -> {
                return new DebugSubscription.Event(subscription, object);
