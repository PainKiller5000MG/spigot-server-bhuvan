...
public class WorldSpawnDataFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("WorldSpawnDataFix", this.getInputSchema().getType(References.LEVEL), (typed) -> {
            return typed.update(DSL.remainderFinder(), (dynamic) -> {
                ...
                Dynamic<?> dynamic1 = dynamic.emptyMap().set("dimension", dynamic.createString("minecraft:overworld")).set("pos", dynamic.createIntList(IntStream.of(new int[]{i, j, k}))).set("yaw", dynamic.createFloat(f)).set("pitch", dynamic.createFloat(0.0F));
