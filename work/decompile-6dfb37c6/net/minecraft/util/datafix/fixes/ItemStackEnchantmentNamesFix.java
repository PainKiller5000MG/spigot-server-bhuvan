...
public class ItemStackEnchantmentNamesFix extends DataFix {
    ...
    private Dynamic<?> fixTag(Dynamic<?> tag) {
        DataResult dataresult = tag.get("ench").asStreamOpt().map((stream) -> {
        ...
        Optional<? extends Dynamic<?>> optional = dataresult.map(tag::createList).result();

        if (optional.isPresent()) {
            tag = tag.remove("ench").set("Enchantments", (Dynamic) optional.get());
        }

        return tag.update("StoredEnchantments", (dynamic1) -> {
