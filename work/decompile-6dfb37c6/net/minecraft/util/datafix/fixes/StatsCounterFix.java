...
public class StatsCounterFix extends DataFix {
    ...
    private static StatsCounterFix.@Nullable StatType unpackLegacyKey(String key) {
        if (StatsCounterFix.SKIP.contains(key)) {
            return null;
        } else {
            String s1 = (String) StatsCounterFix.CUSTOM_MAP.get(key);

            if (s1 != null) {
                return new StatsCounterFix.StatType("minecraft:custom", s1);
            } else {
                int i = StringUtils.ordinalIndexOf(key, ".", 2);

                if (i < 0) {
                    return null;
                } else {
                    String s2 = key.substring(0, i);

                    if ("stat.mineBlock".equals(s2)) {
                        String s3 = upgradeBlock(key.substring(i + 1).replace('.', ':'));

                        return new StatsCounterFix.StatType("minecraft:mined", s3);
                    } else {
                        String s4 = (String) StatsCounterFix.ITEM_KEYS.get(s2);

                        if (s4 != null) {
                            String s5 = key.substring(i + 1).replace('.', ':');
                            String s6 = upgradeItem(s5);
                            String s7 = s6 == null ? s5 : s6;

                            return new StatsCounterFix.StatType(s4, s7);
                        } else {
                            String s8 = (String) StatsCounterFix.ENTITY_KEYS.get(s2);

                            if (s8 != null) {
                                String s9 = key.substring(i + 1).replace('.', ':');
                                String s10 = (String) StatsCounterFix.ENTITIES.getOrDefault(s9, s9);

                                return new StatsCounterFix.StatType(s8, s10);
                            } else {
                                return null;
                            }
                        }
    ...
    private TypeRewriteRule makeStatFixer() {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", type, type1, (typed) -> {
            ...
            if (optional.isPresent()) {
                for (Map.Entry<? extends Dynamic<?>, ? extends Dynamic<?>> map_entry : ((Map) optional.get()).entrySet()) {
                    if (((Dynamic) map_entry.getValue()).asNumber().result().isPresent()) {
                        String s = ((Dynamic) map_entry.getKey()).asString("");
                        StatsCounterFix.StatType statscounterfix_stattype = unpackLegacyKey(s);

                        if (statscounterfix_stattype != null) {
                            Dynamic<?> dynamic1 = dynamic.createString(statscounterfix_stattype.type());
                            Dynamic<?> dynamic2 = (Dynamic) map.computeIfAbsent(dynamic1, (dynamic3) -> {
                                return dynamic.emptyMap();
                            });

                            map.put(dynamic1, dynamic2.set(statscounterfix_stattype.typeKey(), (Dynamic) map_entry.getValue()));
                        }
                    }
                }
            }
        ...
        return this.fixTypeEverywhereTyped("ObjectiveStatFix", type, type1, (typed) -> {
            ...
            Dynamic<?> dynamic1 = dynamic.update("CriteriaName", (dynamic2) -> {
                Optional optional = dynamic2.asString().result().map((s) -> {
                    if (StatsCounterFix.SPECIAL_OBJECTIVE_CRITERIA.contains(s)) {
                        return s;
                    } else {
                        StatsCounterFix.StatType statscounterfix_stattype = unpackLegacyKey(s);
                        ...
                        if (statscounterfix_stattype == null) {
                            ...
                            String s1 = V1451_6.packNamespacedWithDot(statscounterfix_stattype.type);

                            return s1 + ":" + V1451_6.packNamespacedWithDot(statscounterfix_stattype.typeKey);
                        }
                    }
                });
