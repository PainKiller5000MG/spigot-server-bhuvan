...
public class ChunkStructuresTemplateRenameFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ChunkStructuresTemplateRenameFix", type, (typed) -> {
            return typed.update(DSL.remainderFinder(), this::fixChildren);
        });
    }

    private Dynamic<?> fixChildren(Dynamic<?> structure) {
        return structure.update("Children", (dynamic1) -> {
            return structure.createList(dynamic1.asStream().map((dynamic2) -> {
                return this.fixTag(structure, dynamic2);
            }));
        });
    }

    private Dynamic<?> fixTag(Dynamic<?> structure, Dynamic<?> child) {
        String s = structure.get("id").asString("");

        if (ChunkStructuresTemplateRenameFix.RENAMES.containsKey(s)) {
            Pair<String, ImmutableMap<String, String>> pair = (Pair) ChunkStructuresTemplateRenameFix.RENAMES.get(s);

            if (((String) pair.getFirst()).equals(child.get("id").asString(""))) {
                String s1 = child.get("Template").asString("");

                child = child.set("Template", child.createString((String) ((ImmutableMap) pair.getSecond()).getOrDefault(s1, s1)));
            }
        }
