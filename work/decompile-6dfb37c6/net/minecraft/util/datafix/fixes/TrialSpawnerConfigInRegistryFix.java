...
public class TrialSpawnerConfigInRegistryFix extends NamedEntityFix {
    ...
    public Dynamic<?> fixTag(Dynamic<Tag> input) {
        Optional<Dynamic<Tag>> optional = input.get("normal_config").result();

        if (optional.isEmpty()) {
            return input;
        } else {
            Optional<Dynamic<Tag>> optional1 = input.get("ominous_config").result();

            if (optional1.isEmpty()) {
                return input;
            } else {
                Identifier identifier = (Identifier) TrialSpawnerConfigInRegistryFix.VanillaTrialChambers.CONFIGS_TO_KEY.get(Pair.of((Dynamic) optional.get(), (Dynamic) optional1.get()));

                return identifier == null ? input : input.set("normal_config", input.createString(identifier.withSuffix("/normal").toString())).set("ominous_config", input.createString(identifier.withSuffix("/ominous").toString()));
            }
        }
    }
    ...
    private static final class VanillaTrialChambers {
        ...
        private static void register(Identifier location, String normalNbt, String ominousNbt) {
            try {
                CompoundTag compoundtag = parse(normalNbt);
                CompoundTag compoundtag1 = parse(ominousNbt);
                ...
                TrialSpawnerConfigInRegistryFix.VanillaTrialChambers.CONFIGS_TO_KEY.put(Pair.of(dynamic, asDynamic(compoundtag1)), location);
                TrialSpawnerConfigInRegistryFix.VanillaTrialChambers.CONFIGS_TO_KEY.put(Pair.of(dynamic, asDynamic(compoundtag2)), location);
                TrialSpawnerConfigInRegistryFix.VanillaTrialChambers.CONFIGS_TO_KEY.put(Pair.of(dynamic, asDynamic(compoundtag3)), location);
            } catch (RuntimeException runtimeexception) {
                throw new IllegalStateException("Failed to parse NBT for " + String.valueOf(location), runtimeexception);
            }
        }
