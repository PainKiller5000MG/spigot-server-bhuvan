...
public class VillagerRebuildLevelAndXpFix extends DataFix {
    ...
    public static int getMinXpPerLevel(int level) {
        return VillagerRebuildLevelAndXpFix.LEVEL_XP_THRESHOLDS[Mth.clamp(level - 1, 0, VillagerRebuildLevelAndXpFix.LEVEL_XP_THRESHOLDS.length - 1)];
    }
    ...
    private static Typed<?> addLevel(Typed<?> villager, int level) {
        return villager.update(DSL.remainderFinder(), (dynamic) -> {
            return dynamic.update("VillagerData", (dynamic1) -> {
                return dynamic1.set("level", dynamic1.createInt(level));
            });
        });
    }

    private static Typed<?> addXpFromLevel(Typed<?> villager, int level) {
        int j = getMinXpPerLevel(level);
