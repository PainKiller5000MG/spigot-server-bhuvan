...
public class LevelFlatGeneratorInfoFix extends DataFix {
    ...
    private Dynamic<?> fix(Dynamic<?> input) {
        return input.get("generatorName").asString("").equalsIgnoreCase("flat") ? input.update("generatorOptions", (dynamic1) -> {
    ...
    String fixString(String generatorOptions) {
        if (generatorOptions.isEmpty()) {
            ...
            Iterator<String> iterator = LevelFlatGeneratorInfoFix.SPLITTER.split(generatorOptions).iterator();
            ...
            if (i >= 0 && i <= 3) {
                ...
                Splitter splitter = i < 3 ? LevelFlatGeneratorInfoFix.OLD_AMOUNT_SPLITTER : LevelFlatGeneratorInfoFix.AMOUNT_SPLITTER;

                stringbuilder.append((String) StreamSupport.stream(LevelFlatGeneratorInfoFix.LAYER_SPLITTER.split(s2).spliterator(), false).map((s3) -> {
                    ...
                    List<String> list1 = LevelFlatGeneratorInfoFix.BLOCK_SPLITTER.splitToList(s4);
                    int k = ((String) list1.get(0)).equals("minecraft") ? 1 : 0;
                    String s5 = (String) list1.get(k);
                    int l = i == 3 ? EntityBlockStateFix.getBlockId("minecraft:" + s5) : NumberUtils.toInt(s5, 0);
                    int i1 = k + 1;
                    int j1 = list1.size() > i1 ? NumberUtils.toInt((String) list1.get(i1), 0) : 0;
                    String s6 = j == 1 ? "" : j + "*";

                    return s6 + BlockStateData.getTag(l << 4 | j1).get("Name").asString("");
