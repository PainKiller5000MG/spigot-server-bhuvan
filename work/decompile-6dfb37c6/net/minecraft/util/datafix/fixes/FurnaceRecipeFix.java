...
public class FurnaceRecipeFix extends DataFix {
    ...
    private <R> TypeRewriteRule cap(Type<R> recipeType) {
        Type<Pair<Either<Pair<List<Pair<R, Integer>>, Dynamic<?>>, Unit>, Dynamic<?>>> type1 = DSL.and(DSL.optional(DSL.field("RecipesUsed", DSL.and(DSL.compoundList(recipeType, DSL.intType()), DSL.remainderType()))), DSL.remainderType());
        ...
        return this.fixTypeEverywhereTyped("FurnaceRecipesFix", type5, type6, (typed) -> {
            return typed.updateTyped(opticfinder, type2, (typed1) -> {
                return this.updateFurnaceContents(recipeType, type1, typed1);
    ...
    private <R> Typed<?> updateFurnaceContents(Type<R> recipeType, Type<Pair<Either<Pair<List<Pair<R, Integer>>, Dynamic<?>>, Unit>, Dynamic<?>>> replacedType, Typed<?> input) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (k > 0) {
                optional.ifPresent((dynamic1) -> {
                    Optional<? extends Pair<R, ? extends Dynamic<?>>> optional1 = recipeType.read(dynamic1).result();
        ...
        return input.set(DSL.remainderFinder(), replacedType, Pair.of(Either.left(Pair.of(list, dynamic.emptyMap())), dynamic));
    }
}
