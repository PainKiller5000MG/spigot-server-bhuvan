...
public class EntityRidingToPassengersFix extends DataFix {
    ...
    private <OldEntityTree, NewEntityTree, Entity> TypeRewriteRule cap(Schema inputSchema, Schema outputType, Type<OldEntityTree> oldEntityTreeType, Type<NewEntityTree> newEntityTreeType, Type<Entity> entityType) {
        Type<Pair<String, Pair<Either<OldEntityTree, Unit>, Entity>>> type3 = DSL.named(References.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Riding", oldEntityTreeType)), entityType));
        Type<Pair<String, Pair<Either<List<NewEntityTree>, Unit>, Entity>>> type4 = DSL.named(References.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Passengers", DSL.list(newEntityTreeType))), entityType));
        Type<?> type5 = inputSchema.getType(References.ENTITY_TREE);
        Type<?> type6 = outputType.getType(References.ENTITY_TREE);
        ...
        if (!Objects.equals(type5, type3)) {
            ...
            Type<?> type7 = inputSchema.getType(References.PLAYER);
            Type<?> type8 = outputType.getType(References.PLAYER);
                ...
                return (pair) -> {
                    ...
                    while (true) {
                        Either<List<NewEntityTree>, Unit> either = (Either) DataFixUtils.orElse(optional.map((pair2) -> {
                            Typed<NewEntityTree> typed = (Typed) newEntityTreeType.pointTyped(dynamicops).orElseThrow(() -> {
                                return new IllegalStateException("Could not create new entity tree");
                            });
                            NewEntityTree newentitytree = (NewEntityTree) typed.set(opticfinder1, pair2).getOptional(opticfinder2).orElseThrow(() -> {
                        ...
                        pair1 = (Pair) (new Typed(oldEntityTreeType, dynamicops, optional1.get())).getOptional(opticfinder).orElseThrow(() -> {
