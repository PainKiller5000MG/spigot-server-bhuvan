...
public class BlockEntityCustomNameToComponentFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("BlockEntityCustomNameToComponentFix", type, type1, (typed) -> {
            ...
            return optional.isPresent() && !BlockEntityCustomNameToComponentFix.NAMEABLE_BLOCK_ENTITIES.contains(optional.get()) ? ExtraDataFixUtils.cast(type1, typed) : Util.writeAndReadTypedOrThrow(ExtraDataFixUtils.cast(type2, typed), type1, BlockEntityCustomNameToComponentFix::fixTagCustomName);
        });
    }
    ...
    public static <T> Dynamic<T> fixTagCustomName(Dynamic<T> tag) {
        ...
        return s.isEmpty() ? tag.remove("CustomName") : tag.set("CustomName", LegacyComponentDataFixUtils.createPlainTextComponent(tag.getOps(), s));
    }
}
