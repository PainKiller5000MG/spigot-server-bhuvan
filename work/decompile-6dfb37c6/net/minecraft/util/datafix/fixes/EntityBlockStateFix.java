...
public class EntityBlockStateFix extends DataFix {
    ...
    public static int getBlockId(String name) {
        Integer integer = (Integer) EntityBlockStateFix.MAP.get(name);
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(References.ENTITY), schema1.getType(References.ENTITY), (typed) -> {
            typed = this.updateEntity(typed, "minecraft:falling_block", this::updateFallingBlock);
            typed = this.updateEntity(typed, "minecraft:enderman", (typed1) -> {
                return this.updateBlockToBlockState(typed1, "carried", "carriedData", "carriedBlockState");
            });
            typed = this.updateEntity(typed, "minecraft:arrow", function1);
            typed = this.updateEntity(typed, "minecraft:spectral_arrow", function1);
            typed = this.updateEntity(typed, "minecraft:egg", function2);
            typed = this.updateEntity(typed, "minecraft:ender_pearl", function2);
            typed = this.updateEntity(typed, "minecraft:fireball", function2);
            typed = this.updateEntity(typed, "minecraft:potion", function2);
            typed = this.updateEntity(typed, "minecraft:small_fireball", function2);
            typed = this.updateEntity(typed, "minecraft:snowball", function2);
            typed = this.updateEntity(typed, "minecraft:wither_skull", function2);
            typed = this.updateEntity(typed, "minecraft:xp_bottle", function2);
            ...
            typed = this.updateEntity(typed, "minecraft:spawner_minecart", function);
            return typed;
        });
    }
    ...
    private Typed<?> updateFallingBlock(Typed<?> input) {
        ...
        Dynamic<?> dynamic = (Dynamic) input.get(DSL.remainderFinder());

        return input.update(type.finder(), type1, (either) -> {
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> input, String oldFieldName, String dataName, String newFieldName) {
        Type<Pair<String, Either<Integer, String>>> type = DSL.field(oldFieldName, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())));
        Type<Pair<String, Dynamic<?>>> type1 = DSL.field(newFieldName, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
        Dynamic<?> dynamic = (Dynamic) input.getOrCreate(DSL.remainderFinder());

        return input.update(type.finder(), type1, (pair) -> {
            int i = (Integer) ((Either) pair.getSecond()).map((integer) -> {
                return integer;
            }, EntityBlockStateFix::getBlockId);
            int j = dynamic.get(dataName).asInt(0) & 15;

            return Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(i << 4 | j));
        }).set(DSL.remainderFinder(), dynamic.remove(dataName));
    }
    ...
    private Typed<?> updateEntity(Typed<?> input, String name, Function<Typed<?>, Typed<?>> function) {
        ...
        return input.updateTyped(DSL.namedChoice(name, type), type1, function);
    }
}
