...
public class DropInvalidSignDataFix extends DataFix {
    ...
    private static <T> Dynamic<T> fixText(Dynamic<T> tag) {
        Optional<Stream<Dynamic<T>>> optional = tag.get("filtered_messages").asStreamOpt().result();

        if (optional.isEmpty()) {
            return tag;
        } else {
            Dynamic<T> dynamic1 = LegacyComponentDataFixUtils.<T>createEmptyComponent(tag.getOps());
            List<Dynamic<T>> list = ((Stream) tag.get("messages").asStreamOpt().result().orElse(Stream.of())).toList();
            ...
            return list1.equals(list) ? tag.remove("filtered_messages") : tag.set("filtered_messages", tag.createList(list1.stream()));
        }
    }
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("DropInvalidSignDataFix for " + this.entityName, type, (typed) -> {
            return typed.updateTyped(opticfinder, type1, (typed1) -> {
                ...
                }) : Util.writeAndReadTypedOrThrow(typed1, type1, this::fix);
            });
        });
    }
}
