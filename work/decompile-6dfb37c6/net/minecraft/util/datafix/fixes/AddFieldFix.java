...
public class AddFieldFix extends DataFix {
    ...
    private Dynamic<?> addField(Dynamic<?> dynamic, int pathIndex) {
        if (pathIndex >= this.path.length) {
            ...
            Optional<? extends Dynamic<?>> optional = dynamic.get(this.path[pathIndex]).result();

            return optional.isEmpty() ? dynamic : this.addField((Dynamic) optional.get(), pathIndex + 1);
        }
    }
}
