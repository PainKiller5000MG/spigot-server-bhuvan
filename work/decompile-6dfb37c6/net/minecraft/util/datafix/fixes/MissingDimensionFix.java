...
public class MissingDimensionFix extends DataFix {
                ...
                return typed.updateTyped(fieldfinder, (typed1) -> {
                    return typed1.updateTyped(opticfinder, (typed2) -> {
                        if (!(typed2.getValue() instanceof List)) {
                            ...
                            return (Typed) DataFixUtils.orElse(compoundlist_compoundlisttype.readTyped(dynamic1).result().map(Pair::getFirst), typed2);
                        } else {
                            return typed2;
                        }
                    });
    ...
    private <T> Dynamic<T> recreateSettings(Dynamic<T> tag) {
        long i = tag.get("seed").asLong(0L);

        return new Dynamic(tag.getOps(), WorldGenSettingsFix.vanillaLevels(tag, i, WorldGenSettingsFix.defaultOverworld(tag, i), false));
    }
}
