...
public abstract class FloatProvider implements SampledFloat {
    ...
    public static Codec<FloatProvider> codec(float minValue, float maxValue) {
        return FloatProvider.CODEC.validate((floatprovider) -> {
            return floatprovider.getMinValue() < minValue ? DataResult.error(() -> {
                return "Value provider too low: " + minValue + " [" + floatprovider.getMinValue() + "-" + floatprovider.getMaxValue() + "]";
            }) : (floatprovider.getMaxValue() > maxValue ? DataResult.error(() -> {
                return "Value provider too high: " + maxValue + " [" + floatprovider.getMinValue() + "-" + floatprovider.getMaxValue() + "]";
