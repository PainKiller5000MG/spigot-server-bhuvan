...
public class SimpleBitStorage implements BitStorage {
    ...
    public SimpleBitStorage(int bits, int size, long @Nullable [] data) {
        ...
        if (data != null) {
            if (data.length != l) {
                throw new SimpleBitStorage.InitializationException("Invalid length given for storage, got: " + data.length + " but expected: " + l);
            }
    ...
    private int cellIndex(int bitIndex) {
        ...
        return (int) ((long) bitIndex * j + k >> 32 >> this.divideShift);
    }
