...
public class ActiveProfiler implements ProfileCollector {
    ...
    public void endTick() {
        if (!this.started) {
            ...
            if (!this.path.isEmpty()) {
                ActiveProfiler.LOGGER.error("Profiler tick ended before path was fully popped (remainder: '{}'). Mismatched push/pop?", LogUtils.defer(() -> {
    ...
    public void push(String name) {
        if (!this.started) {
            ActiveProfiler.LOGGER.error("Cannot push '{}' to profiler if profiler tick hasn't started - missing startTick()?", name);
        } else {
            if (!this.path.isEmpty()) {
                this.path = this.path + "\u001e";
            }
    ...
    public void pop() {
        if (!this.started) {
            ...
            if (k > ActiveProfiler.WARNING_TIME_NANOS && !this.suppressWarnings.getAsBoolean()) {
                ActiveProfiler.LOGGER.warn("Something's taking too long! '{}' took aprox {} ms", LogUtils.defer(() -> {
                    return ProfileResults.demanglePath(this.path);
                }), LogUtils.defer(() -> {
    ...
    private ActiveProfiler.PathEntry getCurrentEntry() {
        if (this.currentEntry == null) {
            this.currentEntry = (ActiveProfiler.PathEntry) this.entries.computeIfAbsent(this.path, (s) -> {
