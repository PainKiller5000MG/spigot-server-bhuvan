...
public interface JvmProfiler {
    ...
    JvmProfiler INSTANCE = (JvmProfiler) (Runtime.class.getModule().getLayer().findModule("jdk.jfr").isPresent() && FlightRecorder.isAvailable() ? JfrProfiler.getInstance() : new JvmProfiler.NoOpProfiler());
    ...
    void onPacketReceived(ConnectionProtocol protocol, PacketType<?> packetId, SocketAddress remoteAddress, int readableBytes);

    void onPacketSent(ConnectionProtocol protocol, PacketType<?> packetId, SocketAddress remoteAddress, int writtenBytes);
    ...
    public static class NoOpProfiler implements JvmProfiler {
        ...
        public boolean start(Environment environment) {
            JvmProfiler.NoOpProfiler.LOGGER.warn("Attempted to start Flight Recorder, but it's not supported on this JVM");
            return false;
        }
        ...
        public ProfiledDuration onWorldLoadedStarted() {
            return JvmProfiler.NoOpProfiler.noOpCommit;
        }
