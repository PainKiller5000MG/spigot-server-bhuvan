...
public final class IoSummary<T> {
    ...
    public IoSummary(Duration recordingDuration, List<Pair<T, IoSummary.CountAndSize>> packetStats) {
        ...
        this.totalCountAndSize = (IoSummary.CountAndSize) packetStats.stream().map(Pair::getSecond).reduce(new IoSummary.CountAndSize(0L, 0L), IoSummary.CountAndSize::add);
    ...
    public static record CountAndSize(long totalCount, long totalSize) {
        ...
        private static final Comparator<IoSummary.CountAndSize> SIZE_THEN_COUNT = Comparator.comparing(IoSummary.CountAndSize::totalSize).thenComparing(IoSummary.CountAndSize::totalCount).reversed();
