...
public record ChatTypeDecoration(String translationKey, List<ChatTypeDecoration.Parameter> parameters, Style style) {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, ChatTypeDecoration> STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.STRING_UTF8, ChatTypeDecoration::translationKey, ChatTypeDecoration.Parameter.STREAM_CODEC.apply(ByteBufCodecs.list()), ChatTypeDecoration::parameters, Style.Serializer.TRUSTED_STREAM_CODEC, ChatTypeDecoration::style, ChatTypeDecoration::new);
    ...
    public Component decorate(Component content, ChatType.Bound chatType) {
        Object[] aobject = this.resolveParameters(content, chatType);
    ...
    private Component[] resolveParameters(Component content, ChatType.Bound chatType) {
        ...
        for (int i = 0; i < acomponent.length; ++i) {
            ChatTypeDecoration.Parameter chattypedecoration_parameter = (ChatTypeDecoration.Parameter) this.parameters.get(i);

            acomponent[i] = chattypedecoration_parameter.select(content, chatType);
        }
    ...
    public static enum Parameter implements StringRepresentable {
        ...
        SENDER(0, "sender", (component, chattype_bound) -> {
            return chattype_bound.name();
        }), TARGET(1, "target", (component, chattype_bound) -> {
            return (Component) chattype_bound.targetName().orElse(CommonComponents.EMPTY);
        }), CONTENT(2, "content", (component, chattype_bound) -> {
            return component;
        });

        private static final IntFunction<ChatTypeDecoration.Parameter> BY_ID = ByIdMap.<ChatTypeDecoration.Parameter>continuous((chattypedecoration_parameter) -> {
            return chattypedecoration_parameter.id;
        }, values(), ByIdMap.OutOfBoundsStrategy.ZERO);
        public static final Codec<ChatTypeDecoration.Parameter> CODEC = StringRepresentable.<ChatTypeDecoration.Parameter>fromEnum(ChatTypeDecoration.Parameter::values);
        public static final StreamCodec<ByteBuf, ChatTypeDecoration.Parameter> STREAM_CODEC = ByteBufCodecs.idMapper(ChatTypeDecoration.Parameter.BY_ID, (chattypedecoration_parameter) -> {
            return chattypedecoration_parameter.id;
        });
        ...
        private Parameter(int id, String name, ChatTypeDecoration.Parameter.Selector selector) {
            this.id = id;
            this.name = name;
