    ...
    public static class KeyBased implements SignedMessageValidator {
        ...
        private boolean validateChain(PlayerChatMessage message) {
            if (message.equals(this.lastMessage)) {
                ...
                SignedMessageValidator.KeyBased.LOGGER.error("Received out-of-order chat message from {}: expected index > {} for session {}, but was {} for session {}", new Object[]{message.sender(), this.lastMessage.link().index(), this.lastMessage.link().sessionId(), message.link().index(), message.link().sessionId()});
                return false;
            } else {
                return true;
            }
        ...
        private boolean validate(PlayerChatMessage message) {
            if (this.expired.getAsBoolean()) {
                SignedMessageValidator.KeyBased.LOGGER.error("Received message with expired profile public key from {} with session {}", message.sender(), message.link().sessionId());
                return false;
            } else if (!message.verify(this.validator)) {
                SignedMessageValidator.KeyBased.LOGGER.error("Received message with invalid signature (is the session wrong, or signature cache out of sync?): {}", PlayerChatMessage.describeSigned(message));
                return false;
            } else {
