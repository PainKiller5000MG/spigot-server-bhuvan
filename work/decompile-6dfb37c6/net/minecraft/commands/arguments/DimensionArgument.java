...
public class DimensionArgument implements ArgumentType<Identifier> {
    ...
    private static final Collection<String> EXAMPLES = (Collection) Stream.of(Level.OVERWORLD, Level.NETHER).map((resourcekey) -> {
    ...
    private static final DynamicCommandExceptionType ERROR_INVALID_VALUE = new DynamicCommandExceptionType((object) -> {
        return Component.translatableEscape("argument.dimension.invalid", object);
    });
    ...
    public static ServerLevel getDimension(CommandContext<CommandSourceStack> context, String name) throws CommandSyntaxException {
        ...
        ServerLevel serverlevel = ((CommandSourceStack) context.getSource()).getServer().getLevel(resourcekey);

        if (serverlevel == null) {
            throw DimensionArgument.ERROR_INVALID_VALUE.create(identifier);
        } else {
            return serverlevel;
        }
    }
