...
public record LocalCoordinates(double left, double up, double forwards) implements Coordinates {
    ...
    public Vec3 getPosition(CommandSourceStack sender) {
        Vec3 vec3 = sender.getAnchor().apply(sender);

        return Vec3.applyLocalCoordinatesToRotation(sender.getRotation(), new Vec3(this.left, this.up, this.forwards)).add(vec3.x, vec3.y, vec3.z);
    }
