...
public class ResourceOrIdArgument<T> implements ArgumentType<Holder<T>> {
    ...
    public static final DynamicCommandExceptionType ERROR_FAILED_TO_PARSE = new DynamicCommandExceptionType((object) -> {
        return Component.translatableEscape("argument.resource_or_id.failed_to_parse", object);
    });
    public static final Dynamic2CommandExceptionType ERROR_NO_SUCH_ELEMENT = new Dynamic2CommandExceptionType((object, object1) -> {
        return Component.translatableEscape("argument.resource_or_id.no_such_element", object, object1);
    });
    ...
    protected ResourceOrIdArgument(CommandBuildContext context, ResourceKey<? extends Registry<T>> registryKey, Codec<T> codec) {
        ...
        this.codec = codec;
        this.grammar = createGrammar(registryKey, ResourceOrIdArgument.OPS);
    }
    ...
    public static <T, O> Grammar<ResourceOrIdArgument.Result<T, O>> createGrammar(ResourceKey<? extends Registry<T>> registryKey, DynamicOps<O> ops) {
        Grammar<O> grammar = SnbtGrammar.<O>createParser(ops);
        Dictionary<StringReader> dictionary = new Dictionary<StringReader>();
        Atom<ResourceOrIdArgument.Result<T, O>> atom = Atom.<ResourceOrIdArgument.Result<T, O>>of("result");
        Atom<Identifier> atom1 = Atom.<Identifier>of("id");
        Atom<O> atom2 = Atom.<O>of("value");

        dictionary.put(atom1, IdentifierParseRule.INSTANCE);
        dictionary.put(atom2, grammar.top().value());
        NamedRule<StringReader, ResourceOrIdArgument.Result<T, O>> namedrule = dictionary.put(atom, Term.alternative(dictionary.named(atom1), dictionary.named(atom2)), (scope) -> {
            Identifier identifier = (Identifier) scope.get(atom1);

            if (identifier != null) {
                return new ResourceOrIdArgument.ReferenceResult(ResourceKey.create(registryKey, identifier));
            } else {
                O o0 = (O) scope.getOrThrow(atom2);

                return new ResourceOrIdArgument.InlineResult(o0);
            }
        });

        return new Grammar<ResourceOrIdArgument.Result<T, O>>(dictionary, namedrule);
    }
    ...
    private <O> @Nullable Holder<T> parse(StringReader reader, Grammar<ResourceOrIdArgument.Result<T, O>> grammar, DynamicOps<O> ops) throws CommandSyntaxException {
        ResourceOrIdArgument.Result<T, O> resourceoridargument_result = (ResourceOrIdArgument.Result) grammar.parseForCommands(reader);

        return this.elementLookup.isEmpty() ? null : resourceoridargument_result.parse(reader, this.registryLookup, ops, this.codec, (HolderLookup.RegistryLookup) this.elementLookup.get());
    }
    ...
    public static record InlineResult<T, O>(O value) implements ResourceOrIdArgument.Result<T, O> {
        ...
        public Holder<T> parse(ImmutableStringReader reader, HolderLookup.Provider lookup, DynamicOps<O> ops, Codec<T> codec, HolderLookup.RegistryLookup<T> elementLookup) throws CommandSyntaxException {
            return Holder.<T>direct(codec.parse(lookup.createSerializationContext(ops), this.value).getOrThrow((s) -> {
                return ResourceOrIdArgument.ERROR_FAILED_TO_PARSE.createWithContext(reader, s);
            }));
        }
    }
    ...
    public static record ReferenceResult<T, O>(ResourceKey<T> key) implements ResourceOrIdArgument.Result<T, O> {
        ...
        @Override
        public Holder<T> parse(ImmutableStringReader reader, HolderLookup.Provider lookup, DynamicOps<O> ops, Codec<T> codec, HolderLookup.RegistryLookup<T> elementLookup) throws CommandSyntaxException {
            return (Holder) elementLookup.get(this.key).orElseThrow(() -> {
                return ResourceOrIdArgument.ERROR_NO_SUCH_ELEMENT.createWithContext(reader, this.key.identifier(), this.key.registry());
            });
        }
    }
