...
public class SlotArgument implements ArgumentType<Integer> {
    ...
    private static final DynamicCommandExceptionType ERROR_UNKNOWN_SLOT = new DynamicCommandExceptionType((object) -> {
        return Component.translatableEscape("slot.unknown", object);
    });
    private static final DynamicCommandExceptionType ERROR_ONLY_SINGLE_SLOT_ALLOWED = new DynamicCommandExceptionType((object) -> {
        return Component.translatableEscape("slot.only_single_allowed", object);
    });
    ...
    public Integer parse(StringReader reader) throws CommandSyntaxException {
        String s = ParserUtils.readWhile(reader, (c0) -> {
            return c0 != ' ';
        });
        SlotRange slotrange = SlotRanges.nameToIds(s);

        if (slotrange == null) {
            throw SlotArgument.ERROR_UNKNOWN_SLOT.createWithContext(reader, s);
        } else if (slotrange.size() != 1) {
            throw SlotArgument.ERROR_ONLY_SINGLE_SLOT_ALLOWED.createWithContext(reader, s);
        } else {
