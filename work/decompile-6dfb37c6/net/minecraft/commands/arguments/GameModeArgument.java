...
public class GameModeArgument implements ArgumentType<GameType> {
    ...
    private static final Collection<String> EXAMPLES = (Collection) Stream.of(GameType.SURVIVAL, GameType.CREATIVE).map(GameType::getName).collect(Collectors.toList());
    ...
    private static final DynamicCommandExceptionType ERROR_INVALID = new DynamicCommandExceptionType((object) -> {
        return Component.translatableEscape("argument.gamemode.invalid", object);
    });
    ...
    public GameType parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (gametype == null) {
            throw GameModeArgument.ERROR_INVALID.createWithContext(reader, s);
        } else {
            return gametype;
        }
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return context.getSource() instanceof SharedSuggestionProvider ? SharedSuggestionProvider.suggest(Arrays.stream(GameModeArgument.VALUES).map(GameType::getName), builder) : Suggestions.empty();
    }
