...
public record TagPredicate<T>(TagKey<T> tag, boolean expected) {
    ...
    public static <T> Codec<TagPredicate<T>> codec(ResourceKey<? extends Registry<T>> registryKey) {
        return RecordCodecBuilder.create((instance) -> {
            return instance.group(TagKey.codec(registryKey).fieldOf("id").forGetter(TagPredicate::tag), Codec.BOOL.fieldOf("expected").forGetter(TagPredicate::expected)).apply(instance, TagPredicate::new);
        });
    }
