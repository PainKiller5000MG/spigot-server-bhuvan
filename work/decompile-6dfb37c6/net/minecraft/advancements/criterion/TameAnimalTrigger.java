...
public class TameAnimalTrigger extends SimpleCriterionTrigger<TameAnimalTrigger.TriggerInstance> {
    ...
    public void trigger(ServerPlayer player, Animal animal) {
        LootContext lootcontext = EntityPredicate.createContext(player, animal);

        this.trigger(player, (tameanimaltrigger_triggerinstance) -> {
            return tameanimaltrigger_triggerinstance.matches(lootcontext);
        });
    }
    ...
    public static record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<ContextAwarePredicate> entity) implements SimpleCriterionTrigger.SimpleInstance {
        ...
        public static final Codec<TameAnimalTrigger.TriggerInstance> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(EntityPredicate.ADVANCEMENT_CODEC.optionalFieldOf("player").forGetter(TameAnimalTrigger.TriggerInstance::player), EntityPredicate.ADVANCEMENT_CODEC.optionalFieldOf("entity").forGetter(TameAnimalTrigger.TriggerInstance::entity)).apply(instance, TameAnimalTrigger.TriggerInstance::new);
        });
