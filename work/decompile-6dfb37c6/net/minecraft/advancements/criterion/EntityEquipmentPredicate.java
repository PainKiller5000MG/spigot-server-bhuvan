...
public record EntityEquipmentPredicate(Optional<ItemPredicate> head, Optional<ItemPredicate> chest, Optional<ItemPredicate> legs, Optional<ItemPredicate> feet, Optional<ItemPredicate> body, Optional<ItemPredicate> mainhand, Optional<ItemPredicate> offhand) {
    ...
    public static EntityEquipmentPredicate captainPredicate(HolderGetter<Item> items, HolderGetter<BannerPattern> patternGetter) {
        return EntityEquipmentPredicate.Builder.equipment().head(ItemPredicate.Builder.item().of(items, Items.WHITE_BANNER).withComponents(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.someOf(Raid.getOminousBannerInstance(patternGetter).getComponents(), DataComponents.BANNER_PATTERNS, DataComponents.ITEM_NAME)).build())).build();
    }

    public boolean matches(@Nullable Entity entity) {
        if (entity instanceof LivingEntity livingentity) {
            return this.head.isPresent() && !((ItemPredicate) this.head.get()).test(livingentity.getItemBySlot(EquipmentSlot.HEAD)) ? false : (this.chest.isPresent() && !((ItemPredicate) this.chest.get()).test(livingentity.getItemBySlot(EquipmentSlot.CHEST)) ? false : (this.legs.isPresent() && !((ItemPredicate) this.legs.get()).test(livingentity.getItemBySlot(EquipmentSlot.LEGS)) ? false : (this.feet.isPresent() && !((ItemPredicate) this.feet.get()).test(livingentity.getItemBySlot(EquipmentSlot.FEET)) ? false : (this.body.isPresent() && !((ItemPredicate) this.body.get()).test(livingentity.getItemBySlot(EquipmentSlot.BODY)) ? false : (this.mainhand.isPresent() && !((ItemPredicate) this.mainhand.get()).test(livingentity.getItemBySlot(EquipmentSlot.MAINHAND)) ? false : !this.offhand.isPresent() || ((ItemPredicate) this.offhand.get()).test(livingentity.getItemBySlot(EquipmentSlot.OFFHAND)))))));
        } else {
            return false;
        }
    }
