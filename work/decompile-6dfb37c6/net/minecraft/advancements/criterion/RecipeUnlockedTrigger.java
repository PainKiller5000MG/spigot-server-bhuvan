...
public class RecipeUnlockedTrigger extends SimpleCriterionTrigger<RecipeUnlockedTrigger.TriggerInstance> {
    ...
    public void trigger(ServerPlayer player, RecipeHolder<?> recipe) {
        this.trigger(player, (recipeunlockedtrigger_triggerinstance) -> {
            return recipeunlockedtrigger_triggerinstance.matches(recipe);
        });
    }
    ...
    public static record TriggerInstance(Optional<ContextAwarePredicate> player, ResourceKey<Recipe<?>> recipe) implements SimpleCriterionTrigger.SimpleInstance {
        ...
        public static final Codec<RecipeUnlockedTrigger.TriggerInstance> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(EntityPredicate.ADVANCEMENT_CODEC.optionalFieldOf("player").forGetter(RecipeUnlockedTrigger.TriggerInstance::player), Recipe.KEY_CODEC.fieldOf("recipe").forGetter(RecipeUnlockedTrigger.TriggerInstance::recipe)).apply(instance, RecipeUnlockedTrigger.TriggerInstance::new);
        });
