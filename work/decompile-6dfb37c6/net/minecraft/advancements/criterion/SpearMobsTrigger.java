...
public class SpearMobsTrigger extends SimpleCriterionTrigger<SpearMobsTrigger.TriggerInstance> {
    ...
    public void trigger(ServerPlayer player, int number) {
        this.trigger(player, (spearmobstrigger_triggerinstance) -> {
            return spearmobstrigger_triggerinstance.matches(number);
        });
    }
    ...
    public static record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<Integer> count) implements SimpleCriterionTrigger.SimpleInstance {
        ...
        public static Criterion<SpearMobsTrigger.TriggerInstance> spearMobs(int requiredCount) {
