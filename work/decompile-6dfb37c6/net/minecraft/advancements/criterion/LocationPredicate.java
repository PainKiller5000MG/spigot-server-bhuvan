...
public record LocationPredicate(Optional<LocationPredicate.PositionPredicate> position, Optional<HolderSet<Biome>> biomes, Optional<HolderSet<Structure>> structures, Optional<ResourceKey<Level>> dimension, Optional<Boolean> smokey, Optional<LightPredicate> light, Optional<BlockPredicate> block, Optional<FluidPredicate> fluid, Optional<Boolean> canSeeSky) {
    ...
    public static final Codec<LocationPredicate> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(LocationPredicate.PositionPredicate.CODEC.optionalFieldOf("position").forGetter(LocationPredicate::position), RegistryCodecs.homogeneousList(Registries.BIOME).optionalFieldOf("biomes").forGetter(LocationPredicate::biomes), RegistryCodecs.homogeneousList(Registries.STRUCTURE).optionalFieldOf("structures").forGetter(LocationPredicate::structures), ResourceKey.codec(Registries.DIMENSION).optionalFieldOf("dimension").forGetter(LocationPredicate::dimension), Codec.BOOL.optionalFieldOf("smokey").forGetter(LocationPredicate::smokey), LightPredicate.CODEC.optionalFieldOf("light").forGetter(LocationPredicate::light), BlockPredicate.CODEC.optionalFieldOf("block").forGetter(LocationPredicate::block), FluidPredicate.CODEC.optionalFieldOf("fluid").forGetter(LocationPredicate::fluid), Codec.BOOL.optionalFieldOf("can_see_sky").forGetter(LocationPredicate::canSeeSky)).apply(instance, LocationPredicate::new);
    });
    ...
    public boolean matches(ServerLevel level, double x, double y, double z) {
        if (this.position.isPresent() && !((LocationPredicate.PositionPredicate) this.position.get()).matches(x, y, z)) {
            ...
            return !this.biomes.isPresent() || flag && ((HolderSet) this.biomes.get()).contains(level.getBiome(blockpos)) ? (!this.structures.isPresent() || flag && level.structureManager().getStructureWithPieceAt(blockpos, (HolderSet) this.structures.get()).isValid() ? (!this.smokey.isPresent() || flag && (Boolean) this.smokey.get() == CampfireBlock.isSmokeyPos(level, blockpos) ? (this.light.isPresent() && !((LightPredicate) this.light.get()).matches(level, blockpos) ? false : (this.block.isPresent() && !((BlockPredicate) this.block.get()).matches(level, blockpos) ? false : (this.fluid.isPresent() && !((FluidPredicate) this.fluid.get()).matches(level, blockpos) ? false : !this.canSeeSky.isPresent() || (Boolean) this.canSeeSky.get() == level.canSeeSky(blockpos)))) : false) : false) : false;
        }
    }
    ...
    private static record PositionPredicate(MinMaxBounds.Doubles x, MinMaxBounds.Doubles y, MinMaxBounds.Doubles z) {
        ...
        public static final Codec<LocationPredicate.PositionPredicate> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(MinMaxBounds.Doubles.CODEC.optionalFieldOf("x", MinMaxBounds.Doubles.ANY).forGetter(LocationPredicate.PositionPredicate::x), MinMaxBounds.Doubles.CODEC.optionalFieldOf("y", MinMaxBounds.Doubles.ANY).forGetter(LocationPredicate.PositionPredicate::y), MinMaxBounds.Doubles.CODEC.optionalFieldOf("z", MinMaxBounds.Doubles.ANY).forGetter(LocationPredicate.PositionPredicate::z)).apply(instance, LocationPredicate.PositionPredicate::new);
        });
