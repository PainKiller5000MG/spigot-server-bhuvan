...
public record FishingHookPredicate(Optional<Boolean> inOpenWater) implements EntitySubPredicate {
    ...
    public static final MapCodec<FishingHookPredicate> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
    ...
    public boolean matches(Entity entity, ServerLevel level, @Nullable Vec3 position) {
        if (this.inOpenWater.isEmpty()) {
            ...
            return (Boolean) this.inOpenWater.get() == fishinghook.isOpenWaterFishing();
        } else {
            return false;
        }
    }
