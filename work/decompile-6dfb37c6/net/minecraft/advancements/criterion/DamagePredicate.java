...
public record DamagePredicate(MinMaxBounds.Doubles dealtDamage, MinMaxBounds.Doubles takenDamage, Optional<EntityPredicate> sourceEntity, Optional<Boolean> blocked, Optional<DamageSourcePredicate> type) {
    ...
    public static final Codec<DamagePredicate> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(MinMaxBounds.Doubles.CODEC.optionalFieldOf("dealt", MinMaxBounds.Doubles.ANY).forGetter(DamagePredicate::dealtDamage), MinMaxBounds.Doubles.CODEC.optionalFieldOf("taken", MinMaxBounds.Doubles.ANY).forGetter(DamagePredicate::takenDamage), EntityPredicate.CODEC.optionalFieldOf("source_entity").forGetter(DamagePredicate::sourceEntity), Codec.BOOL.optionalFieldOf("blocked").forGetter(DamagePredicate::blocked), DamageSourcePredicate.CODEC.optionalFieldOf("type").forGetter(DamagePredicate::type)).apply(instance, DamagePredicate::new);
    });

    public boolean matches(ServerPlayer player, DamageSource source, float originalDamage, float actualDamage, boolean blocked) {
        return !this.dealtDamage.matches((double) originalDamage) ? false : (!this.takenDamage.matches((double) actualDamage) ? false : (this.sourceEntity.isPresent() && !((EntityPredicate) this.sourceEntity.get()).matches(player, source.getEntity()) ? false : (this.blocked.isPresent() && (Boolean) this.blocked.get() != blocked ? false : !this.type.isPresent() || ((DamageSourcePredicate) this.type.get()).matches(player, source))));
    }
