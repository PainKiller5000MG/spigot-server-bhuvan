...
public record FluidPredicate(Optional<HolderSet<Fluid>> fluids, Optional<StatePropertiesPredicate> properties) {
    ...
    public static final Codec<FluidPredicate> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(RegistryCodecs.homogeneousList(Registries.FLUID).optionalFieldOf("fluids").forGetter(FluidPredicate::fluids), StatePropertiesPredicate.CODEC.optionalFieldOf("state").forGetter(FluidPredicate::properties)).apply(instance, FluidPredicate::new);
    });
    ...
    public boolean matches(ServerLevel level, BlockPos pos) {
        if (!level.isLoaded(pos)) {
            ...
            return this.fluids.isPresent() && !fluidstate.is((HolderSet) this.fluids.get()) ? false : !this.properties.isPresent() || ((StatePropertiesPredicate) this.properties.get()).matches(fluidstate);
        }
    }
    ...
    public static class Builder {
        ...
        public FluidPredicate.Builder of(HolderSet<Fluid> fluids) {
