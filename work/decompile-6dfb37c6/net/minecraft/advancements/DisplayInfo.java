...
public class DisplayInfo {
    ...
    private void serializeToNetwork(RegistryFriendlyByteBuf output) {
        ...
        output.writeInt(i);
        Optional optional = this.background.map(ClientAsset::id);

        Objects.requireNonNull(output);
        optional.ifPresent(output::writeIdentifier);
        output.writeFloat(this.x);
        output.writeFloat(this.y);
    }

    private static DisplayInfo fromNetwork(RegistryFriendlyByteBuf input) {
        Component component = (Component) ComponentSerialization.TRUSTED_STREAM_CODEC.decode(input);
        Component component1 = (Component) ComponentSerialization.TRUSTED_STREAM_CODEC.decode(input);
        ItemStack itemstack = (ItemStack) ItemStack.STREAM_CODEC.decode(input);
        AdvancementType advancementtype = (AdvancementType) input.readEnum(AdvancementType.class);
        int i = input.readInt();
        Optional<ClientAsset.ResourceTexture> optional = (i & 1) != 0 ? Optional.of(new ClientAsset.ResourceTexture(input.readIdentifier())) : Optional.empty();
        boolean flag = (i & 2) != 0;
        boolean flag1 = (i & 4) != 0;
        DisplayInfo displayinfo = new DisplayInfo(itemstack, component, component1, optional, advancementtype, flag, false, flag1);

        displayinfo.setLocation(input.readFloat(), input.readFloat());
        return displayinfo;
    }
}
