...
public record TextGizmo(Vec3 pos, String text, TextGizmo.Style style) implements Gizmo {
    ...
    public void emit(GizmoPrimitives primitives, float alphaMultiplier) {
        TextGizmo.Style textgizmo_style;

        if (alphaMultiplier < 1.0F) {
            textgizmo_style = new TextGizmo.Style(ARGB.multiplyAlpha(this.style.color, alphaMultiplier), this.style.scale, this.style.adjustLeft);
        } else {
            textgizmo_style = this.style;
        }

        primitives.addText(this.pos, this.text, textgizmo_style);
    }
    ...
    public static record Style(int color, float scale, OptionalDouble adjustLeft) {
        ...
        public static TextGizmo.Style forColor(int argb) {
            return new TextGizmo.Style(argb, 0.32F, OptionalDouble.of(0.0D));
        }
        ...
        public TextGizmo.Style withLeftAlignment(float adjustLeft) {
            return new TextGizmo.Style(this.color, this.scale, OptionalDouble.of((double) adjustLeft));
        }
    }
}
