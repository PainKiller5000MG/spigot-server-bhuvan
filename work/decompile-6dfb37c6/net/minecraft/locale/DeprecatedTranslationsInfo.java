...
public record DeprecatedTranslationsInfo(List<String> removed, Map<String, String> renamed) {
    ...
    public static DeprecatedTranslationsInfo loadFromResource(String path) {
        try (InputStream inputstream = Language.class.getResourceAsStream(path)) {
            ...
            DeprecatedTranslationsInfo.LOGGER.error("Failed to read {}", path, exception);
        }
    ...
    public void applyToMap(Map<String, String> translations) {
        ...
        this.renamed.forEach((s1, s2) -> {
            String s3 = (String) translations.remove(s1);

            if (s3 == null) {
                DeprecatedTranslationsInfo.LOGGER.warn("Missing translation key for rename: {}", s1);
                translations.remove(s2);
            } else {
                translations.put(s2, s3);
            }
