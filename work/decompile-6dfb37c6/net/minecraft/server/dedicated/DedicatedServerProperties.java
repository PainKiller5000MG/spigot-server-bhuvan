...
public class DedicatedServerProperties extends Settings<DedicatedServerProperties> {
    ...
    public DedicatedServerProperties(Properties settings) {
        super(settings);
        this.difficulty = this.<Difficulty>getMutable("difficulty", dispatchNumberOrString(Difficulty::byId, Difficulty::byName), Difficulty::getKey, Difficulty.EASY);
        this.gameMode = this.<GameType>getMutable("gamemode", dispatchNumberOrString(GameType::byId, GameType::byName), GameType::getName, GameType.SURVIVAL);
        ...
        this.opPermissions = this.<LevelBasedPermissionSet>getMutable("op-permission-level", DedicatedServerProperties::deserializePermission, DedicatedServerProperties::serializePermission, LevelBasedPermissionSet.OWNER);
        ...
        this.entityBroadcastRangePercentage = this.<Integer>getMutable("entity-broadcast-range-percentage", (s) -> {
            return Mth.clamp(Integer.parseInt(s), 10, 1000);
        }, 100);
    ...
    private static @Nullable Component parseResourcePackPrompt(String prompt) {
        if (!Strings.isNullOrEmpty(prompt)) {
            try {
                ...
                return (Component) ComponentSerialization.CODEC.parse(RegistryAccess.EMPTY.createSerializationContext(JsonOps.INSTANCE), jsonelement).resultOrPartial((s1) -> {
                    DedicatedServerProperties.LOGGER.warn("Failed to parse resource pack prompt '{}': {}", prompt, s1);
                }).orElse((Object) null);
    ...
    private static Optional<MinecraftServer.ServerResourcePackInfo> getServerPackInfo(String id, String url, String resourcePackSha1, @Nullable String resourcePackHash, boolean requireResourcePack, String resourcePackPrompt) {
        if (url.isEmpty()) {
            ...
            Component component = parseResourcePackPrompt(resourcePackPrompt);
            UUID uuid;
            ...
            return Optional.of(new MinecraftServer.ServerResourcePackInfo(uuid, url, s5, requireResourcePack, component));
        }
    }

    private static DataPackConfig getDatapackConfig(String enabledPacks, String disabledPacks) {
        List<String> list = DedicatedServerProperties.COMMA_SPLITTER.splitToList(enabledPacks);
        List<String> list1 = DedicatedServerProperties.COMMA_SPLITTER.splitToList(disabledPacks);
    ...
    public static record WorldDimensionData(JsonObject generatorSettings, String levelType) {
        ...
        public WorldDimensions create(HolderLookup.Provider registries) {
            ...
            Holder.Reference<WorldPreset> holder_reference = (Holder.Reference) holderlookup.get(WorldPresets.NORMAL).or(() -> {
                return holderlookup.listElements().findAny();
            }).orElseThrow(() -> {
                return new IllegalStateException("Invalid datapack contents: can't find default preset");
            });
            Optional optional = Optional.ofNullable(Identifier.tryParse(this.levelType)).map((identifier) -> {
                return ResourceKey.create(Registries.WORLD_PRESET, identifier);
            }).or(() -> {
            ...
            Holder<WorldPreset> holder = (Holder) optional.flatMap(holderlookup::get).orElseGet(() -> {
                DedicatedServerProperties.LOGGER.warn("Failed to parse level-type {}, defaulting to {}", this.levelType, holder_reference.key().identifier());
