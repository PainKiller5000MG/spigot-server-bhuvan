    ...
    public void run() {
        while (this.server.isRunning()) {
            ...
            if (k > this.maxTickTimeNanos) {
                ServerWatchdog.LOGGER.error(LogUtils.FATAL_MARKER, "A single server tick took {} seconds (should be max {})", String.format(Locale.ROOT, "%.2f", (float) k / (float) TimeUtil.NANOSECONDS_PER_SECOND), String.format(Locale.ROOT, "%.2f", this.server.tickRateManager().millisecondsPerTick() / (float) TimeUtil.MILLISECONDS_PER_SECOND));
                ServerWatchdog.LOGGER.error(LogUtils.FATAL_MARKER, "Considering it to be crashed, server will forcibly shutdown.");
                ...
                crashreportcategory.setDetail("Random tick rate", () -> {
                    return this.server.getWorldData().getGameRules().getAsString(GameRules.RANDOM_TICK_SPEED);
                });
                crashreportcategory.setDetail("Level stats", () -> {
                    return (String) Streams.stream(this.server.getAllLevels()).map((serverlevel) -> {
                        String s = String.valueOf(serverlevel.dimension().identifier());

                        return s + ": " + serverlevel.getWatchdogStats();
                ...
                if (crashreport.saveToFile(path, ReportType.CRASH)) {
                    ServerWatchdog.LOGGER.error("This crash report has been saved to: {}", path.toAbsolutePath());
                } else {
