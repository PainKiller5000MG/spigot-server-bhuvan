...
public class LoggingLevelLoadListener implements LevelLoadListener {
        ...
        if (!this.closed) {
            ...
            switch (stage) {
                case PREPARE_GLOBAL_SPAWN:
                    LoggingLevelLoadListener.LOGGER.info("Selecting global world spawn...");
                    break;
                case LOAD_INITIAL_CHUNKS:
                    LoggingLevelLoadListener.LOGGER.info("Loading {} persistent chunks...", totalChunks);
                    break;
                case LOAD_PLAYER_CHUNKS:
                    LoggingLevelLoadListener.LOGGER.info("Loading {} chunks for player spawn...", totalChunks);
            }
    ...
    public void update(LevelLoadListener.Stage stage, int currentChunks, int totalChunks) {
        if (!this.closed) {
            ...
            if (Util.getMillis() > this.nextLogTime) {
                ...
                LoggingLevelLoadListener.LOGGER.info(Component.translatable("menu.preparingSpawn", k).getString());
            }
    ...
    public void finish(LevelLoadListener.Stage stage) {
        if (!this.closed) {
            ...
            LevelLoadListener.Stage levelloadlistener_stage1 = this.includePlayerChunks ? LevelLoadListener.Stage.LOAD_PLAYER_CHUNKS : LevelLoadListener.Stage.LOAD_INITIAL_CHUNKS;

            if (stage == levelloadlistener_stage1) {
                LoggingLevelLoadListener.LOGGER.info("Time elapsed: {} ms", Util.getMillis() - this.startTime);
