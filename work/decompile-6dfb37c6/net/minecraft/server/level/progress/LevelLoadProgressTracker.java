        ...
        if (this.tracksStage(stage)) {
            switch (stage) {
                case LOAD_INITIAL_CHUNKS:
                    int j = this.includePlayerChunks ? LevelLoadProgressTracker.EXPECTED_PLAYER_CHUNKS : 0;

                    this.totalWeight = 10 + totalChunks + j;
                    this.beginSegment(10);
                    this.finishSegment();
                    this.beginSegment(totalChunks);
                    break;
                case LOAD_PLAYER_CHUNKS:
                    this.beginSegment(LevelLoadProgressTracker.EXPECTED_PLAYER_CHUNKS);
            }
