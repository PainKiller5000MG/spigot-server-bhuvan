...
public class SynchronizeRegistriesTask implements ConfigurationTask {
    ...
    private void sendRegistries(Consumer<Packet<?>> connection, Set<KnownPack> negotiatedPacks) {
        DynamicOps<Tag> dynamicops = this.registries.compositeAccess().<Tag>createSerializationContext(NbtOps.INSTANCE);

        RegistrySynchronization.packRegistries(dynamicops, this.registries.getAccessFrom(RegistryLayer.WORLDGEN), negotiatedPacks, (resourcekey, list) -> {
            connection.accept(new ClientboundRegistryDataPacket(resourcekey, list));
        });
