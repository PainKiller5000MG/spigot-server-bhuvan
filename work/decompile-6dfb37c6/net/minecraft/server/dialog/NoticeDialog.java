...
public record NoticeDialog(CommonDialogData common, ActionButton action) implements SimpleDialog {
    ...
    public static final MapCodec<NoticeDialog> MAP_CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(CommonDialogData.MAP_CODEC.forGetter(NoticeDialog::common), ActionButton.CODEC.optionalFieldOf("action", NoticeDialog.DEFAULT_ACTION).forGetter(NoticeDialog::action)).apply(instance, NoticeDialog::new);
    });

    @Override
    public MapCodec<NoticeDialog> codec() {
        return NoticeDialog.MAP_CODEC;
    }
