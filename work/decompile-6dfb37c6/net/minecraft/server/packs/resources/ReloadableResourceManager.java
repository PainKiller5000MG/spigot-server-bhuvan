...
public class ReloadableResourceManager implements AutoCloseable, ResourceManager {
    ...
    public ReloadInstance createReload(Executor backgroundExecutor, Executor mainThreadExecutor, CompletableFuture<Unit> initialTask, List<PackResources> resourcePacks) {
        ReloadableResourceManager.LOGGER.info("Reloading ResourceManager: {}", LogUtils.defer(() -> {
            return resourcePacks.stream().map(PackResources::packId).collect(Collectors.joining(", "));
        }));
        this.resources.close();
        this.resources = new MultiPackResourceManager(this.type, resourcePacks);
        return SimpleReloadInstance.create(this.resources, this.listeners, backgroundExecutor, mainThreadExecutor, initialTask, ReloadableResourceManager.LOGGER.isDebugEnabled());
    }
