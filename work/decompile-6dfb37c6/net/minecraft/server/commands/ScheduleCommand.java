...
public class ScheduleCommand {
    ...
    private static int schedule(CommandSourceStack source, Pair<Identifier, Either<CommandFunction<CommandSourceStack>, Collection<CommandFunction<CommandSourceStack>>>> callback, int time, boolean replace) throws CommandSyntaxException {
        if (time == 0) {
            ...
            Identifier identifier = (Identifier) callback.getFirst();
            TimerQueue<MinecraftServer> timerqueue = source.getServer().getWorldData().overworldData().getScheduledEvents();
            Optional<CommandFunction<CommandSourceStack>> optional = ((Either) callback.getSecond()).left();
            ...
            if (optional.isPresent()) {
                ...
                timerqueue.schedule(s, j, new FunctionCallback(identifier));
                source.sendSuccess(() -> {
                    return Component.translatable("commands.schedule.created.function", Component.translationArg(identifier), time, j);
                }, true);
            } else {
                ...
                timerqueue.schedule(s1, j, new FunctionTagCallback(identifier));
                source.sendSuccess(() -> {
                    return Component.translatable("commands.schedule.created.tag", Component.translationArg(identifier), time, j);
                }, true);
            }
    ...
    private static int remove(CommandSourceStack source, String id) throws CommandSyntaxException {
        int i = source.getServer().getWorldData().overworldData().getScheduledEvents().remove(id);

        if (i == 0) {
            throw ScheduleCommand.ERROR_CANT_REMOVE.create(id);
        } else {
            source.sendSuccess(() -> {
                return Component.translatable("commands.schedule.cleared.success", i, id);
            }, true);
            return i;
        }
    }
