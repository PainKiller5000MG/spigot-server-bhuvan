...
public record PlayerDto(Optional<UUID> id, Optional<String> name) {
    ...
    public static final MapCodec<PlayerDto> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(UUIDUtil.STRING_CODEC.optionalFieldOf("id").forGetter(PlayerDto::id), Codec.STRING.optionalFieldOf("name").forGetter(PlayerDto::name)).apply(instance, PlayerDto::new);
    });
