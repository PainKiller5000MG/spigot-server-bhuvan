...
public class JsonRpcApiSchema implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        DiscoveryService.DiscoverResponse discoveryservice_discoverresponse = DiscoveryService.discover(Schema.getSchemaRegistry());

        return DataProvider.saveStable(cache, (JsonElement) DiscoveryService.DiscoverResponse.CODEC.codec().encodeStart(JsonOps.INSTANCE, discoveryservice_discoverresponse).getOrThrow(), this.path);
    }
