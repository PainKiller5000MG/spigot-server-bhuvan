...
public class IncomingRpcMethods {
    ...
    public static IncomingRpcMethod<?, ?> bootstrap(Registry<IncomingRpcMethod<?, ?>> methodRegistry) {
        ...
        return IncomingRpcMethod.method((minecraftapi) -> {
            return DiscoveryService.discover(Schema.getSchemaRegistry());
        }).undiscoverable().notOnMainThread().response("result", Schema.DISCOVERY_SCHEMA).register(methodRegistry, "rpc.discover");
    }
