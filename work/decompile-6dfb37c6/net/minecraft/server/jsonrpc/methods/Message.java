...
public record Message(Optional<String> literal, Optional<String> translatable, Optional<List<String>> translatableParams) {
    ...
    public static final Codec<Message> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(Codec.STRING.optionalFieldOf("literal").forGetter(Message::literal), Codec.STRING.optionalFieldOf("translatable").forGetter(Message::translatable), Codec.STRING.listOf().lenientOptionalFieldOf("translatableParams").forGetter(Message::translatableParams)).apply(instance, Message::new);
    });

    public Optional<Component> asComponent() {
        if (this.translatable.isPresent()) {
            String s = (String) this.translatable.get();

            if (this.translatableParams.isPresent()) {
                List<String> list = (List) this.translatableParams.get();
