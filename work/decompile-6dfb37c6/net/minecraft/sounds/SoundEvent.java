...
public record SoundEvent(Identifier location, Optional<Float> fixedRange) {
    ...
    public static final Codec<SoundEvent> DIRECT_CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(Identifier.CODEC.fieldOf("sound_id").forGetter(SoundEvent::location), Codec.FLOAT.lenientOptionalFieldOf("range").forGetter(SoundEvent::fixedRange)).apply(instance, SoundEvent::create);
    });
    public static final Codec<Holder<SoundEvent>> CODEC = RegistryFileCodec.<Holder<SoundEvent>>create(Registries.SOUND_EVENT, SoundEvent.DIRECT_CODEC);
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, Holder<SoundEvent>> STREAM_CODEC = ByteBufCodecs.holder(Registries.SOUND_EVENT, SoundEvent.DIRECT_STREAM_CODEC);

    private static SoundEvent create(Identifier location, Optional<Float> range) {
        return (SoundEvent) range.map((ofloat) -> {
            return createFixedRangeEvent(location, ofloat);
        }).orElseGet(() -> {
            return createVariableRangeEvent(location);
        });
    }
