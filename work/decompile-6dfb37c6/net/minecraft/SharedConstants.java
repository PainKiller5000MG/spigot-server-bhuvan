...
public class SharedConstants {
    ...
    private static boolean booleanProperty(String name) {
        ...
        return s1 != null && (s1.isEmpty() || Boolean.parseBoolean(s1));
    }
    ...
    private static boolean debugFlag(String name) {
        if (!SharedConstants.DEBUG_ENABLED) {
            ...
            String s1 = prefixDebugFlagName(name);

            if (SharedConstants.DEBUG_PRINT_PROPERTIES) {
                System.out.println("Debug property available: " + s1 + ": bool");
            }

            return booleanProperty(s1);
        }
    }
    ...
    private static int debugIntValue(String name) {
        if (!SharedConstants.DEBUG_ENABLED) {
            ...
            if (SharedConstants.DEBUG_PRINT_PROPERTIES) {
                System.out.println("Debug property available: " + s1 + ": int");
            }

            return Integer.parseInt(System.getProperty(s1, "0"));
        }
    }

    public static void setVersion(WorldVersion version) {
        if (SharedConstants.CURRENT_VERSION == null) {
            SharedConstants.CURRENT_VERSION = version;
        } else if (version != SharedConstants.CURRENT_VERSION) {
    ...
    public static boolean debugVoidTerrain(ChunkPos pos) {
        ...
        return SharedConstants.DEBUG_ONLY_GENERATE_HALF_THE_WORLD ? j < 0 : (!SharedConstants.debugGenerateSquareTerrainWithoutNoise ? false : i > 8192 || i < 0 || j > 1024 || j < 0);
    }

    static {
        ResourceLeakDetector.setLevel(SharedConstants.NETTY_LEAK_DETECTION);
        CommandSyntaxException.ENABLE_COMMAND_STACK_TRACES = SharedConstants.COMMAND_STACK_TRACES;
