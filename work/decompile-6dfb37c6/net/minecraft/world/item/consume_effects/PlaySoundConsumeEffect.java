...
public record PlaySoundConsumeEffect(Holder<SoundEvent> sound) implements ConsumeEffect {
    ...
    public static final MapCodec<PlaySoundConsumeEffect> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(SoundEvent.CODEC.fieldOf("sound").forGetter(PlaySoundConsumeEffect::sound)).apply(instance, PlaySoundConsumeEffect::new);
    });
    ...
    public boolean apply(Level level, ItemStack stack, LivingEntity user) {
        level.playSound((Entity) null, user.blockPosition(), this.sound.value(), user.getSoundSource(), 1.0F, 1.0F);
        return true;
    }
}
