...
public class FireworkStarRecipe extends CustomRecipe {
        ...
        if (input.ingredientCount() < 2) {
            ...
            for (int i = 0; i < input.size(); ++i) {
                ...
                if (!itemstack.isEmpty()) {
                    if (FireworkStarRecipe.SHAPE_BY_ITEM.containsKey(itemstack.getItem())) {
                        if (flag2) {
                            return false;
                        }

                        flag2 = true;
                    } else if (FireworkStarRecipe.TWINKLE_INGREDIENT.test(itemstack)) {
                        if (flag4) {
                            return false;
                        }

                        flag4 = true;
                    } else if (FireworkStarRecipe.TRAIL_INGREDIENT.test(itemstack)) {
                        if (flag3) {
                            return false;
                        }

                        flag3 = true;
                    } else if (FireworkStarRecipe.GUNPOWDER_INGREDIENT.test(itemstack)) {
                        if (flag) {
                            return false;
                        }
    ...
    public ItemStack assemble(CraftingInput input, HolderLookup.Provider registries) {
        FireworkExplosion.Shape fireworkexplosion_shape = FireworkExplosion.Shape.SMALL_BALL;
        boolean flag = false;
        boolean flag1 = false;
        ...
        for (int i = 0; i < input.size(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
                FireworkExplosion.Shape fireworkexplosion_shape1 = (FireworkExplosion.Shape) FireworkStarRecipe.SHAPE_BY_ITEM.get(itemstack.getItem());

                if (fireworkexplosion_shape1 != null) {
                    fireworkexplosion_shape = fireworkexplosion_shape1;
                } else if (FireworkStarRecipe.TWINKLE_INGREDIENT.test(itemstack)) {
                    flag = true;
                } else if (FireworkStarRecipe.TRAIL_INGREDIENT.test(itemstack)) {
                    flag1 = true;
        ...
        itemstack1.set(DataComponents.FIREWORK_EXPLOSION, new FireworkExplosion(fireworkexplosion_shape, intlist, IntList.of(), flag1, flag));
        return itemstack1;
    }
