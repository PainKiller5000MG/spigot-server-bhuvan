...
public class DecoratedPotRecipe extends CustomRecipe {
    ...
    public boolean matches(CraftingInput input, Level level) {
        return input.width() == 3 && input.height() == 3 && input.ingredientCount() == 4 ? back(input).is(ItemTags.DECORATED_POT_INGREDIENTS) && left(input).is(ItemTags.DECORATED_POT_INGREDIENTS) && right(input).is(ItemTags.DECORATED_POT_INGREDIENTS) && front(input).is(ItemTags.DECORATED_POT_INGREDIENTS) : false;
    }
