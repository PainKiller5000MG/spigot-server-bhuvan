...
public record TransmuteResult(Holder<Item> item, int count, DataComponentPatch components) {
    ...
    private static DataResult<TransmuteResult> validate(TransmuteResult result) {
        return ItemStack.validateStrict(new ItemStack(result.item, result.count, result.components)).map((itemstack) -> {
            return result;
        });
    }

    public ItemStack apply(ItemStack input) {
        ItemStack itemstack1 = input.transmuteCopy(this.item.value(), this.count);
    ...
    public boolean isResultUnchanged(ItemStack input) {
        ItemStack itemstack1 = this.apply(input);

        return itemstack1.getCount() == 1 && ItemStack.isSameItemSameComponents(input, itemstack1);
    }
