...
public class RecipePropertySet {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, RecipePropertySet> STREAM_CODEC = Item.STREAM_CODEC.apply(ByteBufCodecs.list()).map((list) -> {
    ...
    private static ResourceKey<RecipePropertySet> registerVanilla(String name) {
        return ResourceKey.create(RecipePropertySet.TYPE_KEY, Identifier.withDefaultNamespace(name));
    }
    ...
    static RecipePropertySet create(Collection<Ingredient> ingredients) {
        Set<Holder<Item>> set = (Set) ingredients.stream().flatMap(Ingredient::items).collect(Collectors.toUnmodifiableSet());
