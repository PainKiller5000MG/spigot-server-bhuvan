...
public record Equippable(EquipmentSlot slot, Holder<SoundEvent> equipSound, Optional<ResourceKey<EquipmentAsset>> assetId, Optional<Identifier> cameraOverlay, Optional<HolderSet<EntityType<?>>> allowedEntities, boolean dispensable, boolean swappable, boolean damageOnHurt, boolean equipOnInteract, boolean canBeSheared, Holder<SoundEvent> shearingSound) {
    ...
    public static Equippable saddle() {
        ...
        return builder(EquipmentSlot.SADDLE).setEquipSound(SoundEvents.HORSE_SADDLE).setAsset(EquipmentAssets.SADDLE).setAllowedEntities(holdergetter.getOrThrow(EntityTypeTags.CAN_EQUIP_SADDLE)).setEquipOnInteract(true).setCanBeSheared(true).setShearingSound(SoundEvents.SADDLE_UNEQUIP).build();
    }

    public static Equippable harness(DyeColor color) {
        HolderGetter<EntityType<?>> holdergetter = BuiltInRegistries.<EntityType<?>>acquireBootstrapRegistrationLookup(BuiltInRegistries.ENTITY_TYPE);

        return builder(EquipmentSlot.BODY).setEquipSound(SoundEvents.HARNESS_EQUIP).setAsset((ResourceKey) EquipmentAssets.HARNESSES.get(color)).setAllowedEntities(holdergetter.getOrThrow(EntityTypeTags.CAN_EQUIP_HARNESS)).setEquipOnInteract(true).setCanBeSheared(true).setShearingSound(BuiltInRegistries.SOUND_EVENT.wrapAsHolder(SoundEvents.HARNESS_UNEQUIP)).build();
    }
    ...
    public InteractionResult swapWithEquipmentSlot(ItemStack inHand, Player player) {
        if (player.canUseSlot(this.slot) && this.canBeEquippedBy(player.getType())) {
            ItemStack itemstack1 = player.getItemBySlot(this.slot);

            if ((!EnchantmentHelper.has(itemstack1, EnchantmentEffectComponents.PREVENT_ARMOR_CHANGE) || player.isCreative()) && !ItemStack.isSameItemSameComponents(inHand, itemstack1)) {
                if (!player.level().isClientSide()) {
                    player.awardStat(Stats.ITEM_USED.get(inHand.getItem()));
                }

                if (inHand.getCount() <= 1) {
                    ItemStack itemstack2 = itemstack1.isEmpty() ? inHand : itemstack1.copyAndClear();
                    ItemStack itemstack3 = player.isCreative() ? inHand.copy() : inHand.copyAndClear();
                    ...
                    ItemStack itemstack5 = inHand.consumeAndReturn(1, player);

                    player.setItemSlot(this.slot, itemstack5);
    ...
    public boolean canBeEquippedBy(EntityType<?> type) {
        return this.allowedEntities.isEmpty() || ((HolderSet) this.allowedEntities.get()).contains(type.builtInRegistryHolder());
    }
