...
public record TrimPattern(Identifier assetId, Component description, boolean decal) {
    ...
    public static final Codec<TrimPattern> DIRECT_CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(Identifier.CODEC.fieldOf("asset_id").forGetter(TrimPattern::assetId), ComponentSerialization.CODEC.fieldOf("description").forGetter(TrimPattern::description), Codec.BOOL.fieldOf("decal").orElse(false).forGetter(TrimPattern::decal)).apply(instance, TrimPattern::new);
    });
    ...
    public static final Codec<Holder<TrimPattern>> CODEC = RegistryFileCodec.<Holder<TrimPattern>>create(Registries.TRIM_PATTERN, TrimPattern.DIRECT_CODEC);
