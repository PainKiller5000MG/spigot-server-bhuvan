...
public record Repairable(HolderSet<Item> items) {
    ...
    public static final Codec<Repairable> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(RegistryCodecs.homogeneousList(Registries.ITEM).fieldOf("items").forGetter(Repairable::items)).apply(instance, Repairable::new);
    });
