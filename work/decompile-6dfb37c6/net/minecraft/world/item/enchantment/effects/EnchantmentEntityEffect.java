...
public interface EnchantmentEntityEffect extends EnchantmentLocationBasedEffect {
    ...
    static MapCodec<? extends EnchantmentEntityEffect> bootstrap(Registry<MapCodec<? extends EnchantmentEntityEffect>> registry) {
        ...
        return (MapCodec) Registry.register(registry, "summon_entity", SummonEntityEffect.CODEC);
    }

    void apply(ServerLevel serverLevel, int enchantmentLevel, EnchantedItemInUse item, Entity entity, Vec3 position);

    @Override
    default void onChangedBlock(ServerLevel serverLevel, int enchantmentLevel, EnchantedItemInUse item, Entity entity, Vec3 position, boolean becameActive) {
