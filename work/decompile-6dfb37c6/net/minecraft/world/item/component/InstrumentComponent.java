...
public record InstrumentComponent(EitherHolder<Instrument> instrument) implements TooltipProvider {
    ...
    public void addToTooltip(Item.TooltipContext context, Consumer<Component> consumer, TooltipFlag flag, DataComponentGetter components) {
        ...
        if (holderlookup_provider != null) {
            this.unwrap(holderlookup_provider).ifPresent((holder) -> {
                Component component = ComponentUtils.mergeStyles(((Instrument) holder.value()).description(), Style.EMPTY.withColor(ChatFormatting.GRAY));
