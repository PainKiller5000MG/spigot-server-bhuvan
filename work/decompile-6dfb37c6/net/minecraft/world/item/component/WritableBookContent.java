...
public record WritableBookContent(List<Filterable<String>> pages) implements BookContent<String, WritableBookContent> {
    ...
    public static final Codec<List<Filterable<String>>> PAGES_CODEC = WritableBookContent.PAGE_CODEC.sizeLimitedListOf(100);
    public static final Codec<WritableBookContent> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(WritableBookContent.PAGES_CODEC.optionalFieldOf("pages", List.of()).forGetter(WritableBookContent::pages)).apply(instance, WritableBookContent::new);
    });
    ...
    public Stream<String> getPages(boolean filterEnabled) {
        return this.pages.stream().map((filterable) -> {
            return (String) filterable.get(filterEnabled);
        });
    }
