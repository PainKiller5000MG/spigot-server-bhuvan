...
public record SuspiciousStewEffects(List<SuspiciousStewEffects.Entry> effects) implements ConsumableListener, TooltipProvider {
    ...
    public void onConsume(Level level, LivingEntity user, ItemStack stack, Consumable consumable) {
        for (SuspiciousStewEffects.Entry suspicioussteweffects_entry : this.effects) {
            user.addEffect(suspicioussteweffects_entry.createEffectInstance());
        }
    ...
    public void addToTooltip(Item.TooltipContext context, Consumer<Component> consumer, TooltipFlag flag, DataComponentGetter components) {
        if (flag.isCreative()) {
            ...
            for (SuspiciousStewEffects.Entry suspicioussteweffects_entry : this.effects) {
                list.add(suspicioussteweffects_entry.createEffectInstance());
            }

            PotionContents.addPotionTooltip(list, consumer, 1.0F, context.tickRate());
        }
