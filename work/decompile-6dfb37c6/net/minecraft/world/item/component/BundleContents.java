...
public final class BundleContents implements TooltipComponent {
    ...
    private static Fraction getWeight(ItemStack stack) {
        ...
        if (bundlecontents != null) {
            ...
            List<BeehiveBlockEntity.Occupant> list = ((Bees) stack.getOrDefault(DataComponents.BEES, Bees.EMPTY)).bees();
    ...
    public static class Mutable {
        ...
        private int getMaxAmountToAdd(ItemStack item) {
            ...
            return Math.max(fraction.divideBy(BundleContents.getWeight(item)).intValue(), 0);
        }
        ...
        public int tryInsert(ItemStack itemsToAdd) {
            if (!BundleContents.canItemBeInBundle(itemsToAdd)) {
                ...
                int i = Math.min(itemsToAdd.getCount(), this.getMaxAmountToAdd(itemsToAdd));

                if (i == 0) {
                    return 0;
                } else {
                    this.weight = this.weight.add(BundleContents.getWeight(itemsToAdd).multiplyBy(Fraction.getFraction(i, 1)));
                    int j = this.findStackIndex(itemsToAdd);
                    ...
                    if (j != -1) {
                        ...
                        this.items.add(0, itemsToAdd.split(i));
                    }
