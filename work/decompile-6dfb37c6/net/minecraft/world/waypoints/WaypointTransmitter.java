...
public interface WaypointTransmitter extends Waypoint {
    ...
    static boolean doesSourceIgnoreReceiver(LivingEntity source, ServerPlayer receiver) {
        if (receiver.isSpectator()) {
            ...
            double d0 = Math.min(source.getAttributeValue(Attributes.WAYPOINT_TRANSMIT_RANGE), receiver.getAttributeValue(Attributes.WAYPOINT_RECEIVE_RANGE));

            return (double) source.distanceTo(receiver) >= d0;
        } else {
            return true;
        }
    }
