...
public class ObjectiveCriteria {
    ...
    public static final Codec<ObjectiveCriteria> CODEC = Codec.STRING.comapFlatMap((s) -> {
        return (DataResult) byName(s).map(DataResult::success).orElse(DataResult.error(() -> {
            return "No scoreboard criteria with name: " + s;
        }));
    ...
    private static ObjectiveCriteria registerCustom(String name, boolean readOnly, ObjectiveCriteria.RenderType renderType) {
        ...
        ObjectiveCriteria.CUSTOM_CRITERIA.put(name, objectivecriteria);
    ...
    protected ObjectiveCriteria(String name, boolean readOnly, ObjectiveCriteria.RenderType renderType) {
        ...
        ObjectiveCriteria.CRITERIA_CACHE.put(name, this);
    }

    public static Set<String> getCustomCriteriaNames() {
        return ImmutableSet.copyOf(ObjectiveCriteria.CUSTOM_CRITERIA.keySet());
    }

    public static Optional<ObjectiveCriteria> byName(String name) {
        ObjectiveCriteria objectivecriteria = (ObjectiveCriteria) ObjectiveCriteria.CRITERIA_CACHE.get(name);
        ...
        if (objectivecriteria != null) {
            ...
            return i < 0 ? Optional.empty() : BuiltInRegistries.STAT_TYPE.getOptional(Identifier.bySeparator(name.substring(0, i), '.')).flatMap((stattype) -> {
                return getStat(stattype, Identifier.bySeparator(name.substring(i + 1), '.'));
            });
        }
    }

    private static <T> Optional<ObjectiveCriteria> getStat(StatType<T> statType, Identifier key) {
        Optional optional = statType.getRegistry().getOptional(key);
    ...
    public static enum RenderType implements StringRepresentable {
        ...
        public static final StringRepresentable.EnumCodec<ObjectiveCriteria.RenderType> CODEC = StringRepresentable.<ObjectiveCriteria.RenderType>fromEnum(ObjectiveCriteria.RenderType::values);
        ...
        public static ObjectiveCriteria.RenderType byId(String key) {
            return (ObjectiveCriteria.RenderType) ObjectiveCriteria.RenderType.CODEC.byName(key, ObjectiveCriteria.RenderType.INTEGER);
        }
    }
}
