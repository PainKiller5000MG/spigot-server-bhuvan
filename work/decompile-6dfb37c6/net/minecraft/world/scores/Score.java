...
public class Score implements ReadOnlyScoreInfo {
    ...
    public static record Packed(int value, boolean locked, Optional<Component> display, Optional<NumberFormat> numberFormat) {
        ...
        public static final MapCodec<Score.Packed> MAP_CODEC = RecordCodecBuilder.mapCodec((instance) -> {
            return instance.group(Codec.INT.optionalFieldOf("Score", 0).forGetter(Score.Packed::value), Codec.BOOL.optionalFieldOf("Locked", false).forGetter(Score.Packed::locked), ComponentSerialization.CODEC.optionalFieldOf("display").forGetter(Score.Packed::display), NumberFormatTypes.CODEC.optionalFieldOf("format").forGetter(Score.Packed::numberFormat)).apply(instance, Score.Packed::new);
        });
    }
}
