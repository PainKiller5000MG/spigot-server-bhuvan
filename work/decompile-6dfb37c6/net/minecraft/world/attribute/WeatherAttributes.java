...
public class WeatherAttributes {
    ...
    private static final Set<EnvironmentAttribute<?>> WEATHER_ATTRIBUTES = Sets.union(WeatherAttributes.RAIN.keySet(), WeatherAttributes.THUNDER.keySet());
    ...
    public static void addBuiltinLayers(EnvironmentAttributeSystem.Builder system, WeatherAttributes.WeatherAccess weatherAccess) {
        for (EnvironmentAttribute<?> environmentattribute : WeatherAttributes.WEATHER_ATTRIBUTES) {
    ...
    private static <Value> void addLayer(EnvironmentAttributeSystem.Builder system, WeatherAttributes.WeatherAccess weatherAccess, EnvironmentAttribute<Value> attribute) {
        EnvironmentAttributeMap.Entry<Value, ?> environmentattributemap_entry = WeatherAttributes.RAIN.get(attribute);
        EnvironmentAttributeMap.Entry<Value, ?> environmentattributemap_entry1 = WeatherAttributes.THUNDER.get(attribute);

        system.addTimeBasedLayer(attribute, (object, i) -> {
            float f = weatherAccess.thunderLevel();
            float f1 = weatherAccess.rainLevel() - f;

            if (environmentattributemap_entry != null && f1 > 0.0F) {
                Value value = environmentattributemap_entry.applyModifier(object);

                object = attribute.type().stateChangeLerp().apply(f1, object, value);
            }

            if (environmentattributemap_entry1 != null && f > 0.0F) {
                Value value1 = environmentattributemap_entry1.applyModifier(object);

                object = attribute.type().stateChangeLerp().apply(f, object, value1);
            }
