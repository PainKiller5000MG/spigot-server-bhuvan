...
public class Illusioner extends SpellcasterIllager implements RangedAttackMob {
        ...
        if (this.level().isClientSide() && this.isInvisible()) {
            ...
            if (this.hurtTime != 1 && this.tickCount % 1200 != 0) {
                ...
                for (int k = 0; k < 4; ++k) {
                    ...
                    this.clientSideIllusionOffsets[1][k] = new Vec3((double) (-6.0F + (float) this.random.nextInt(13)) * 0.5D, (double) Math.max(0, this.random.nextInt(6) - 4), (double) (-6.0F + (float) this.random.nextInt(13)) * 0.5D);
                }
    ...
    public Vec3[] getIllusionOffsets(float a) {
        if (this.clientSideIllusionTicks <= 0) {
            ...
            double d0 = (double) (((float) this.clientSideIllusionTicks - a) / 3.0F);
    ...
    public void performRangedAttack(LivingEntity target, float power) {
        ...
        AbstractArrow abstractarrow = ProjectileUtil.getMobArrow(this, itemstack1, power, itemstack);
    ...
    private class IllusionerBlindnessSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
        ...
        @Override
        public boolean canUse() {
            return !super.canUse() ? false : (Illusioner.this.getTarget() == null ? false : (Illusioner.this.getTarget().getId() == this.lastTargetId ? false : getServerLevel((Entity) Illusioner.this).getCurrentDifficultyAt(Illusioner.this.blockPosition()).isHarderThan((float) Difficulty.NORMAL.ordinal())));
        }
