...
public class Vindicator extends AbstractIllager {
    ...
    protected void customServerAiStep(ServerLevel level) {
        if (!this.isNoAi() && GoalUtils.hasGroundPathNavigation(this)) {
            boolean flag = level.isRaided(this.blockPosition());
    ...
    public @Nullable SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, EntitySpawnReason spawnReason, @Nullable SpawnGroupData groupData) {
        SpawnGroupData spawngroupdata1 = super.finalizeSpawn(level, difficulty, spawnReason, groupData);
    ...
    public void applyRaidBuffs(ServerLevel level, int wave, boolean isCaptain) {
        ...
        if (flag1) {
            ResourceKey<EnchantmentProvider> resourcekey = wave > raid.getNumGroups(Difficulty.NORMAL) ? VanillaEnchantmentProviders.RAID_VINDICATOR_POST_WAVE_5 : VanillaEnchantmentProviders.RAID_VINDICATOR;
    ...
    private static class VindicatorJohnnyAttackGoal extends NearestAttackableTargetGoal<LivingEntity> {
        ...
        public VindicatorJohnnyAttackGoal(Vindicator mob) {
            super(mob, LivingEntity.class, 0, true, true, (livingentity, serverlevel) -> {
