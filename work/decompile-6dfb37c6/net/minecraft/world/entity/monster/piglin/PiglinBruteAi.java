...
public class PiglinBruteAi {
    ...
    protected static void initMemories(PiglinBrute body) {
        GlobalPos globalpos = GlobalPos.of(body.level().dimension(), body.blockPosition());

        body.getBrain().setMemory(MemoryModuleType.HOME, globalpos);
    }
    ...
    private static void initIdleActivity(PiglinBrute body, Brain<PiglinBrute> brain) {
        brain.addActivity(Activity.IDLE, 10, ImmutableList.of(StartAttacking.create(PiglinBruteAi::findNearestValidAttackTarget), createIdleLookBehaviors(), createIdleMovementBehaviors(), SetLookAndInteract.create(EntityType.PLAYER, 4)));
    }

    private static void initFightActivity(PiglinBrute body, Brain<PiglinBrute> brain) {
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel level, AbstractPiglin body) {
        ...
        if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(level, body, (LivingEntity) optional.get())) {
            ...
            Optional<? extends LivingEntity> optional1 = body.getBrain().<LivingEntity>getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER);

            return optional1.isPresent() ? optional1 : body.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_NEMESIS);
        }
    }
    ...
    protected static void maybePlayActivitySound(PiglinBrute body) {
        if ((double) body.level().random.nextFloat() < 0.0125D) {
            playActivitySound(body);
        }
