...
public class WardenAi {
    ...
    private static final BehaviorControl<Warden> DIG_COOLDOWN_SETTER = BehaviorBuilder.create((behaviorbuilder_instance) -> {
        return behaviorbuilder_instance.group(behaviorbuilder_instance.registered(MemoryModuleType.DIG_COOLDOWN)).apply(behaviorbuilder_instance, (memoryaccessor) -> {
            return (serverlevel, warden, i) -> {
                if (behaviorbuilder_instance.tryGet(memoryaccessor).isPresent()) {
    ...
    public static void updateActivity(Warden body) {
        body.getBrain().setActiveActivityToFirstValid(ImmutableList.of(Activity.EMERGE, Activity.DIG, Activity.ROAR, Activity.FIGHT, Activity.INVESTIGATE, Activity.SNIFF, Activity.IDLE));
    }

    protected static Brain<?> makeBrain(Warden warden, Dynamic<?> input) {
        Brain.Provider<Warden> brain_provider = Brain.<Warden>provider(WardenAi.MEMORY_TYPES, WardenAi.SENSOR_TYPES);
        Brain<Warden> brain = brain_provider.makeBrain(input);
    ...
    private static void initEmergeActivity(Brain<Warden> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.EMERGE, 5, ImmutableList.of(new Emerging(WardenAi.EMERGE_DURATION)), MemoryModuleType.IS_EMERGING);
    }

    private static void initDiggingActivity(Brain<Warden> brain) {
        brain.addActivityWithConditions(Activity.DIG, ImmutableList.of(Pair.of(0, new ForceUnmount()), Pair.of(1, new Digging(WardenAi.DIGGING_DURATION))), ImmutableSet.of(Pair.of(MemoryModuleType.ROAR_TARGET, MemoryStatus.VALUE_ABSENT), Pair.of(MemoryModuleType.DIG_COOLDOWN, MemoryStatus.VALUE_ABSENT)));
    }
    ...
    private static void initSniffingActivity(Brain<Warden> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.SNIFF, 5, ImmutableList.of(SetRoarTarget.create(Warden::getEntityAngryAt), new Sniffing(WardenAi.SNIFFING_DURATION)), MemoryModuleType.IS_SNIFFING);
    }
    ...
    private static void initFightActivity(Warden body, Brain<Warden> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(WardenAi.DIG_COOLDOWN_SETTER, StopAttackingIfTargetInvalid.create((serverlevel, livingentity) -> {
            return !body.getAngerLevel().isAngry() || !body.canTargetEntity(livingentity);
        }, WardenAi::onTargetInvalid, false), SetEntityLookTarget.create((livingentity) -> {
            return isTarget(body, livingentity);
        }, (float) body.getAttributeValue(Attributes.FOLLOW_RANGE)), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.2F), new SonicBoom(), MeleeAttack.create(18)), MemoryModuleType.ATTACK_TARGET);
    }

    private static boolean isTarget(Warden body, LivingEntity living) {
        return body.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).filter((livingentity1) -> {
            return livingentity1 == living;
        }).isPresent();
    }
