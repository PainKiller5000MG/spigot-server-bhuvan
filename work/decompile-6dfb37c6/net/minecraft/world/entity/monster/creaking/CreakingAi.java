...
public class CreakingAi {
    ...
    static void initIdleActivity(Brain<Creaking> brain) {
        brain.addActivity(Activity.IDLE, 10, ImmutableList.of(StartAttacking.create((serverlevel, creaking) -> {
    ...
    static void initFightActivity(Creaking body, Brain<Creaking> brain) {
        brain.addActivityWithConditions(Activity.FIGHT, 10, ImmutableList.of(SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0F), MeleeAttack.create(Creaking::canMove, 40), StopAttackingIfTargetInvalid.create((serverlevel, livingentity) -> {
            return !isAttackTargetStillReachable(body, livingentity);
    ...
    private static boolean isAttackTargetStillReachable(Creaking creaking, LivingEntity target) {
        Optional<List<Player>> optional = creaking.getBrain().<List<Player>>getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYERS);
    ...
    public static Brain.Provider<Creaking> brainProvider() {
        return Brain.<Creaking>provider(CreakingAi.MEMORY_TYPES, CreakingAi.SENSOR_TYPES);
    }
