...
public class Shoot extends Behavior<Breeze> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, Breeze breeze) {
        return breeze.getPose() != Pose.STANDING ? false : (Boolean) breeze.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).map((livingentity) -> {
            return isTargetWithinRange(breeze, livingentity);
        }).map((obool) -> {
            if (!obool) {
    ...
    protected void tick(ServerLevel level, Breeze breeze, long timestamp) {
        ...
        if (livingentity != null) {
            ...
            if (!brain.getMemory(MemoryModuleType.BREEZE_SHOOT_CHARGING).isPresent() && !brain.getMemory(MemoryModuleType.BREEZE_SHOOT_RECOVERING).isPresent()) {
                brain.setMemoryWithExpiry(MemoryModuleType.BREEZE_SHOOT_RECOVERING, Unit.INSTANCE, (long) Shoot.SHOOT_RECOVER_DELAY_TICKS);
                double d0 = livingentity.getX() - breeze.getX();
                double d1 = livingentity.getY(livingentity.isPassenger() ? 0.8D : 0.3D) - breeze.getFiringYPosition();
                double d2 = livingentity.getZ() - breeze.getZ();

                Projectile.spawnProjectileUsingShoot(new BreezeWindCharge(breeze, level), level, ItemStack.EMPTY, d0, d1, d2, 0.7F, (float) (5 - level.getDifficulty().getId() * 4));
    ...
    private static boolean isTargetWithinRange(Breeze body, LivingEntity target) {
        double d0 = body.position().distanceToSqr(target.position());
