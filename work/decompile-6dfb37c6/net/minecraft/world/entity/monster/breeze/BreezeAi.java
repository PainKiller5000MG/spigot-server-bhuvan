...
public class BreezeAi {
    ...
    private static void initIdleActivity(Brain<Breeze> brain) {
        brain.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, StartAttacking.create((serverlevel, breeze) -> {
            return breeze.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE);
        })), Pair.of(1, StartAttacking.create((serverlevel, breeze) -> {
    ...
    private static void initFightActivity(Breeze body, Brain<Breeze> brain) {
        ...
        Integer integer = 0;
        BiPredicate bipredicate = Sensor.wasEntityAttackableLastNTicks(body, 100).negate();

        Objects.requireNonNull(bipredicate);
        brain.addActivityWithConditions(activity, ImmutableList.of(Pair.of(integer, StopAttackingIfTargetInvalid.create(bipredicate::test)), Pair.of(1, new Shoot()), Pair.of(2, new LongJump()), Pair.of(3, new ShootWhenStuck()), Pair.of(4, new Slide())), ImmutableSet.of(Pair.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT), Pair.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT)));
    }
