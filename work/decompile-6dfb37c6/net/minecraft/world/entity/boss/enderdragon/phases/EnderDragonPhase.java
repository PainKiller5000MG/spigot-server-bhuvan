...
public class EnderDragonPhase<T extends DragonPhaseInstance> {
    ...
    public static EnderDragonPhase<?> getById(int id) {
        return id >= 0 && id < EnderDragonPhase.phases.length ? EnderDragonPhase.phases[id] : EnderDragonPhase.HOLDING_PATTERN;
    }
    ...
    private static <T extends DragonPhaseInstance> EnderDragonPhase<T> create(Class<T> instanceClass, String name) {
        ...
        EnderDragonPhase.phases = (EnderDragonPhase[]) Arrays.copyOf(EnderDragonPhase.phases, EnderDragonPhase.phases.length + 1);
