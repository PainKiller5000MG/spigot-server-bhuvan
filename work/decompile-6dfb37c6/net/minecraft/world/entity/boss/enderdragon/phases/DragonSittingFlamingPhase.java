...
public class DragonSittingFlamingPhase extends AbstractDragonSittingPhase {
    ...
    public void doClientTick() {
        ...
        if (this.flameTicks % 2 == 0 && this.flameTicks < 10) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                for (int j = 0; j < 6; ++j) {
                    this.dragon.level().addParticle(PowerParticleOption.create(ParticleTypes.DRAGON_BREATH, 1.0F), d3, d4, d5, -vec3.x * (double) 0.08F * (double) j, -vec3.y * (double) 0.6F, -vec3.z * (double) 0.08F * (double) j);
                }
    ...
    public void doServerTick(ServerLevel level) {
        ...
        if (this.flameTicks >= 200) {
            ...
            double d3 = d2;
            BlockPos.MutableBlockPos blockpos_mutableblockpos = new BlockPos.MutableBlockPos(d0, d2, d1);

            while (level.isEmptyBlock(blockpos_mutableblockpos)) {
                --d3;
                if (d3 < 0.0D) {
                    d3 = d2;
                    break;
                ...
                blockpos_mutableblockpos.set(d0, d3, d1);
            }

            d3 = (double) (Mth.floor(d3) + 1);
            this.flame = new AreaEffectCloud(level, d0, d3, d1);
