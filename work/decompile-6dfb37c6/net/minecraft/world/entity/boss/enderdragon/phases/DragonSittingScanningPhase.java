...
public class DragonSittingScanningPhase extends AbstractDragonSittingPhase {
    ...
    public DragonSittingScanningPhase(EnderDragon dragon) {
        super(dragon);
        this.scanTargeting = TargetingConditions.forCombat().range(20.0D).selector((livingentity, serverlevel) -> {
            return Math.abs(livingentity.getY() - dragon.getY()) <= 10.0D;
        });
    }
    ...
    public void doServerTick(ServerLevel level) {
        ...
        if (livingentity != null) {
            if (this.scanningTime > 25) {
                ...
                Vec3 vec31 = (new Vec3((double) Mth.sin((double) (this.dragon.getYRot() * ((float) Math.PI / 180F))), 0.0D, (double) (-Mth.cos((double) (this.dragon.getYRot() * ((float) Math.PI / 180F)))))).normalize();
            ...
            livingentity = level.getNearestPlayer(DragonSittingScanningPhase.CHARGE_TARGETING, this.dragon, this.dragon.getX(), this.dragon.getY(), this.dragon.getZ());
            this.dragon.getPhaseManager().setPhase(EnderDragonPhase.TAKEOFF);
            if (livingentity != null) {
                this.dragon.getPhaseManager().setPhase(EnderDragonPhase.CHARGING_PLAYER);
                ((DragonChargePlayerPhase) this.dragon.getPhaseManager().getPhase(EnderDragonPhase.CHARGING_PLAYER)).setTarget(new Vec3(((LivingEntity) livingentity).getX(), ((LivingEntity) livingentity).getY(), ((LivingEntity) livingentity).getZ()));
            }
        }
