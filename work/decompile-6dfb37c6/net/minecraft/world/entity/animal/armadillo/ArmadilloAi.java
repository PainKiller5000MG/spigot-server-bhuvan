...
public class ArmadilloAi {
    ...
    private static final OneShot<Armadillo> ARMADILLO_ROLLING_OUT = BehaviorBuilder.create((behaviorbuilder_instance) -> {
        return behaviorbuilder_instance.group(behaviorbuilder_instance.absent(MemoryModuleType.DANGER_DETECTED_RECENTLY)).apply(behaviorbuilder_instance, (memoryaccessor) -> {
            return (serverlevel, armadillo, i) -> {
    ...
    public static Brain.Provider<Armadillo> brainProvider() {
        return Brain.<Armadillo>provider(ArmadilloAi.MEMORY_TYPES, ArmadilloAi.SENSOR_TYPES);
    }
    ...
    private static void initCoreActivity(Brain<Armadillo> brain) {
        brain.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new ArmadilloAi.ArmadilloPanic(2.0F), new LookAtTargetSink(45, 90), new MoveToTargetSink() {
            @Override
    ...
    private static void initIdleActivity(Brain<Armadillo> brain) {
        brain.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityType.ARMADILLO, 1.0F, 1)), Pair.of(2, new RunOne(ImmutableList.of(Pair.of(new FollowTemptation((livingentity) -> {
            return 1.25F;
        }, (livingentity) -> {
            return livingentity.isBaby() ? 1.0D : 2.0D;
        }), 1), Pair.of(BabyFollowAdult.create(ArmadilloAi.ADULT_FOLLOW_RANGE, 1.25F), 1)))), Pair.of(3, new RandomLookAround(UniformInt.of(150, 250), 30.0F, 0.0F, 0.0F)), Pair.of(4, new RunOne(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(RandomStroll.stroll(1.0F), 1), Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 1), Pair.of(new DoNothing(30, 60), 1))))));
    }
    ...
    public static class ArmadilloBallUp extends Behavior<Armadillo> {
        ...
        public ArmadilloBallUp() {
            super(Map.of(), ArmadilloAi.ArmadilloBallUp.BALL_UP_STAY_IN_STATE);
        }
        ...
        protected void tick(ServerLevel level, Armadillo body, long timestamp) {
            ...
            if (body.shouldSwitchToScaredState()) {
                ...
                Armadillo.ArmadilloState armadillo_armadillostate = body.getState();
                long j = body.getBrain().getTimeUntilExpiry(MemoryModuleType.DANGER_DETECTED_RECENTLY);
                boolean flag = j > 75L;
                ...
                if (armadillo_armadillostate == Armadillo.ArmadilloState.SCARED) {
                    if (this.nextPeekTimer == 0 && body.onGround() && flag) {
                        level.broadcastEntityEvent(body, (byte) 64);
                        this.nextPeekTimer = this.pickNextPeekTimer(body);
                    }

                    if (j < (long) Armadillo.ArmadilloState.UNROLLING.animationDuration()) {
                        body.playSound(SoundEvents.ARMADILLO_UNROLL_START);
                        body.switchToState(Armadillo.ArmadilloState.UNROLLING);
                    }
                } else if (armadillo_armadillostate == Armadillo.ArmadilloState.UNROLLING && j > (long) Armadillo.ArmadilloState.UNROLLING.animationDuration()) {
                    body.switchToState(Armadillo.ArmadilloState.SCARED);
                }
    ...
    public static class ArmadilloPanic extends AnimalPanic<Armadillo> {
        ...
        public ArmadilloPanic(float speedMultiplier) {
            super(speedMultiplier, (pathfindermob) -> {
