...
public class FrogAi {
    ...
    protected static void initMemories(Frog body, RandomSource random) {
        body.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, FrogAi.TIME_BETWEEN_LONG_JUMPS.sample(random));
    }
    ...
    private static void initIdleActivity(Brain<Frog> brain) {
        brain.addActivityWithConditions(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(0, new AnimalMakeLove(EntityType.FROG)), Pair.of(1, new FollowTemptation((livingentity) -> {
            return 1.25F;
        })), Pair.of(2, StartAttacking.create((serverlevel, frog) -> {
            return canAttack(frog);
        }, (serverlevel, frog) -> {
    ...
    private static void initSwimActivity(Brain<Frog> brain) {
        brain.addActivityWithConditions(Activity.SWIM, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new FollowTemptation((livingentity) -> {
            return 1.25F;
        })), Pair.of(2, StartAttacking.create((serverlevel, frog) -> {
            return canAttack(frog);
        }, (serverlevel, frog) -> {
    ...
    private static void initLaySpawnActivity(Brain<Frog> brain) {
        brain.addActivityWithConditions(Activity.LAY_SPAWN, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, StartAttacking.create((serverlevel, frog) -> {
            return canAttack(frog);
        }, (serverlevel, frog) -> {
            return frog.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE);
    ...
    private static void initJumpActivity(Brain<Frog> brain) {
        brain.addActivityWithConditions(Activity.LONG_JUMP, ImmutableList.of(Pair.of(0, new LongJumpMidJump(FrogAi.TIME_BETWEEN_LONG_JUMPS, SoundEvents.FROG_STEP)), Pair.of(1, new LongJumpToPreferredBlock(FrogAi.TIME_BETWEEN_LONG_JUMPS, 2, 4, 3.5714288F, (frog) -> {
    ...
    private static void initTongueActivity(Brain<Frog> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.TONGUE, 0, ImmutableList.of(StopAttackingIfTargetInvalid.create(), new ShootTongue(SoundEvents.FROG_TONGUE, SoundEvents.FROG_EAT)), MemoryModuleType.ATTACK_TARGET);
    }
    ...
    private static <E extends Mob> boolean isAcceptableLandingSpot(E body, BlockPos targetPos) {
        ...
        if (level.getFluidState(targetPos).isEmpty() && level.getFluidState(blockpos1).isEmpty() && level.getFluidState(targetPos.above()).isEmpty()) {
            BlockState blockstate = level.getBlockState(targetPos);
            BlockState blockstate1 = level.getBlockState(blockpos1);

            if (!blockstate.is(BlockTags.FROG_PREFER_JUMP_TO) && !blockstate1.is(BlockTags.FROG_PREFER_JUMP_TO)) {
                PathfindingContext pathfindingcontext = new PathfindingContext(body.level(), body);
                PathType pathtype = WalkNodeEvaluator.getPathTypeStatic(pathfindingcontext, targetPos.mutable());
                PathType pathtype1 = WalkNodeEvaluator.getPathTypeStatic(pathfindingcontext, blockpos1.mutable());

                return pathtype != PathType.TRAPDOOR && (!blockstate.isAir() || pathtype1 != PathType.TRAPDOOR) ? LongJumpToRandomPos.defaultAcceptableLandingSpot(body, targetPos) : true;
            } else {
                return true;
            }
        } else {
    ...
    private static boolean canAttack(Frog e) {
