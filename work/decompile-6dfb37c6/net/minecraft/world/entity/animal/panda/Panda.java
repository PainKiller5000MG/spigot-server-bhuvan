...
public class Panda extends Animal {
    ...
    private static final EntityDataAccessor<Integer> UNHAPPY_COUNTER = SynchedEntityData.<Integer>defineId(Panda.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> SNEEZE_COUNTER = SynchedEntityData.<Integer>defineId(Panda.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> EAT_COUNTER = SynchedEntityData.<Integer>defineId(Panda.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Byte> MAIN_GENE_ID = SynchedEntityData.<Byte>defineId(Panda.class, EntityDataSerializers.BYTE);
    private static final EntityDataAccessor<Byte> HIDDEN_GENE_ID = SynchedEntityData.<Byte>defineId(Panda.class, EntityDataSerializers.BYTE);
    private static final EntityDataAccessor<Byte> DATA_ID_FLAGS = SynchedEntityData.<Byte>defineId(Panda.class, EntityDataSerializers.BYTE);
    private static final TargetingConditions BREED_TARGETING = TargetingConditions.forNonCombat().range(8.0D);
    ...
    public void setUnhappyCounter(int value) {
        this.entityData.set(Panda.UNHAPPY_COUNTER, value);
    }
    ...
    public void eat(boolean value) {
        this.entityData.set(Panda.EAT_COUNTER, value ? 1 : 0);
    }
    ...
    private void setEatCounter(int value) {
        this.entityData.set(Panda.EAT_COUNTER, value);
    }
    ...
    public void setSneezeCounter(int value) {
        this.entityData.set(Panda.SNEEZE_COUNTER, value);
    }
    ...
    protected void defineSynchedData(SynchedEntityData.Builder entityData) {
        ...
        entityData.define(Panda.UNHAPPY_COUNTER, 0);
        entityData.define(Panda.SNEEZE_COUNTER, 0);
        entityData.define(Panda.MAIN_GENE_ID, (byte) 0);
        entityData.define(Panda.HIDDEN_GENE_ID, (byte) 0);
        entityData.define(Panda.DATA_ID_FLAGS, (byte) 0);
        entityData.define(Panda.EAT_COUNTER, 0);
    }
    ...
    private void setFlag(int flag, boolean value) {
        ...
        if (value) {
            this.entityData.set(Panda.DATA_ID_FLAGS, (byte) (b0 | flag));
        } else {
            this.entityData.set(Panda.DATA_ID_FLAGS, (byte) (b0 & ~flag));
        }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createAnimalAttributes().add(Attributes.MOVEMENT_SPEED, (double) 0.15F).add(Attributes.ATTACK_DAMAGE, 6.0D);
    }
    ...
    private void addEatingParticles() {
        if (this.getEatCounter() % 5 == 0) {
            ...
            for (int i = 0; i < 6; ++i) {
                Vec3 vec3 = new Vec3(((double) this.random.nextFloat() - 0.5D) * 0.1D, (double) this.random.nextFloat() * 0.1D + 0.1D, ((double) this.random.nextFloat() - 0.5D) * 0.1D);
        ...
        if (this.rollCounter > 32) {
            ...
            if (!this.level().isClientSide()) {
                ...
                if (this.rollCounter == 1) {
                    ...
                    this.rollDelta = new Vec3(vec3.x + (double) (-Mth.sin((double) f) * f1), 0.0D, vec3.z + (double) (Mth.cos((double) f) * f1));
    ...
    private void afterSneeze() {
        ...
        level.addParticle(ParticleTypes.SNEEZE, this.getX() - (double) (this.getBbWidth() + 1.0F) * 0.5D * (double) Mth.sin((double) (this.yBodyRot * ((float) Math.PI / 180F))), this.getEyeY() - (double) 0.1F, this.getZ() + (double) (this.getBbWidth() + 1.0F) * 0.5D * (double) Mth.cos((double) (this.yBodyRot * ((float) Math.PI / 180F))), vec3.x, 0.0D, vec3.z);
        this.playSound(SoundEvents.PANDA_SNEEZE, 1.0F, 1.0F);

        for (Panda panda : level.getEntitiesOfClass(Panda.class, this.getBoundingBox().inflate(10.0D))) {
    ...
    public static enum Gene implements StringRepresentable {
        ...
        public static final Codec<Panda.Gene> CODEC = StringRepresentable.<Panda.Gene>fromEnum(Panda.Gene::values);
        private static final IntFunction<Panda.Gene> BY_ID = ByIdMap.<Panda.Gene>continuous(Panda.Gene::getId, values(), ByIdMap.OutOfBoundsStrategy.ZERO);
        ...
        private static Panda.Gene getVariantFromGenes(Panda.Gene mainGene, Panda.Gene hiddenGene) {
    ...
    private class PandaSitGoal extends Goal {
        ...
        public void start() {
            if (Panda.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                List<ItemEntity> list = Panda.this.level().<ItemEntity>getEntitiesOfClass(ItemEntity.class, Panda.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Panda::canPickUpAndEat);

                if (!list.isEmpty()) {
                    Panda.this.getNavigation().moveTo((Entity) list.getFirst(), (double) 1.2F);
                }
            } else {
                Panda.this.tryToSit();
            }
