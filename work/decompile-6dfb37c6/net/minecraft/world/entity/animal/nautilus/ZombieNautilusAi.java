...
public class ZombieNautilusAi {
    ...
    protected static Brain.Provider<ZombieNautilus> brainProvider() {
        return Brain.<ZombieNautilus>provider(ZombieNautilusAi.MEMORY_TYPES, ZombieNautilusAi.SENSOR_TYPES);
    }
    ...
    private static void initCoreActivity(Brain<ZombieNautilus> brain) {
        brain.addActivity(Activity.CORE, 0, ImmutableList.of(new LookAtTargetSink(45, 90), new MoveToTargetSink(), new CountDownCooldownTicks(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.CHARGE_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.ATTACK_TARGET_COOLDOWN)));
    }
