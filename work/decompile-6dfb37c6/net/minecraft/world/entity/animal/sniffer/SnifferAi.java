...
public class SnifferAi {
    ...
    private static void initSniffingActivity(Brain<Sniffer> brain) {
        brain.addActivityWithConditions(Activity.SNIFF, ImmutableList.of(Pair.of(0, new SnifferAi.Searching())), Set.of(Pair.of(MemoryModuleType.IS_PANICKING, MemoryStatus.VALUE_ABSENT), Pair.of(MemoryModuleType.SNIFFER_SNIFFING_TARGET, MemoryStatus.VALUE_PRESENT), Pair.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_PRESENT)));
    }

    private static void initDigActivity(Brain<Sniffer> brain) {
        brain.addActivityWithConditions(Activity.DIG, ImmutableList.of(Pair.of(0, new SnifferAi.Digging(160, 180)), Pair.of(0, new SnifferAi.FinishedDigging(40))), Set.of(Pair.of(MemoryModuleType.IS_PANICKING, MemoryStatus.VALUE_ABSENT), Pair.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), Pair.of(MemoryModuleType.SNIFFER_DIGGING, MemoryStatus.VALUE_PRESENT)));
    }
    ...
    private static class Sniffing extends Behavior<Sniffer> {
        ...
        protected void stop(ServerLevel level, Sniffer sniffer, long timestamp) {
            boolean flag = this.timedOut(timestamp);
    ...
    private static class Searching extends Behavior<Sniffer> {
        ...
        protected boolean canStillUse(ServerLevel level, Sniffer sniffer, long timestamp) {
            if (!sniffer.canSniff()) {
                ...
                Optional<BlockPos> optional1 = sniffer.getBrain().<BlockPos>getMemory(MemoryModuleType.SNIFFER_SNIFFING_TARGET);

                return !optional.isEmpty() && !optional1.isEmpty() ? ((BlockPos) optional1.get()).equals(optional.get()) : false;
            }
        }
