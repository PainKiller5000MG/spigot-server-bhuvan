...
public class GoatAi {
    ...
    private static final TargetingConditions RAM_TARGET_CONDITIONS = TargetingConditions.forCombat().selector((livingentity, serverlevel) -> {
        return !livingentity.getType().equals(EntityType.GOAT) && ((Boolean) serverlevel.getGameRules().get(GameRules.MOB_GRIEFING) || !livingentity.getType().equals(EntityType.ARMOR_STAND)) && serverlevel.getWorldBorder().isWithinBounds(livingentity.getBoundingBox());
    });
    ...
    protected static void initMemories(Goat body, RandomSource random) {
        body.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, GoatAi.TIME_BETWEEN_LONG_JUMPS.sample(random));
        body.getBrain().setMemory(MemoryModuleType.RAM_COOLDOWN_TICKS, GoatAi.TIME_BETWEEN_RAMS.sample(random));
    }
    ...
    private static void initIdleActivity(Brain<Goat> brain) {
        brain.addActivityWithConditions(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(0, new AnimalMakeLove(EntityType.GOAT)), Pair.of(1, new FollowTemptation((livingentity) -> {
            return 1.25F;
        })), Pair.of(2, BabyFollowAdult.create(GoatAi.ADULT_FOLLOW_RANGE, 1.25F)), Pair.of(3, new RunOne(ImmutableList.of(Pair.of(RandomStroll.stroll(1.0F), 2), Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 2), Pair.of(new DoNothing(30, 60), 1))))), ImmutableSet.of(Pair.of(MemoryModuleType.RAM_TARGET, MemoryStatus.VALUE_ABSENT), Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryStatus.VALUE_ABSENT)));
    }

    private static void initLongJumpActivity(Brain<Goat> brain) {
        brain.addActivityWithConditions(Activity.LONG_JUMP, ImmutableList.of(Pair.of(0, new LongJumpMidJump(GoatAi.TIME_BETWEEN_LONG_JUMPS, SoundEvents.GOAT_STEP)), Pair.of(1, new LongJumpToRandomPos(GoatAi.TIME_BETWEEN_LONG_JUMPS, 5, 5, 3.5714288F, (goat) -> {
