...
public class HappyGhastAi {
    ...
    public static Brain.Provider<HappyGhast> brainProvider() {
        return Brain.<HappyGhast>provider(HappyGhastAi.MEMORY_TYPES, HappyGhastAi.SENSOR_TYPES);
    }
    ...
    private static void initCoreActivity(Brain<HappyGhast> brain) {
        brain.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new AnimalPanic(2.0F, 0), new LookAtTargetSink(45, 90), new MoveToTargetSink(), new CountDownCooldownTicks(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS)));
    }
    ...
    private static void initIdleActivity(Brain<HappyGhast> brain) {
        brain.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(1, new FollowTemptation((livingentity) -> {
            ...
            return 3.0D;
        }, true)), Pair.of(2, BabyFollowAdult.create(HappyGhastAi.ADULT_FOLLOW_RANGE, (livingentity) -> {
            return 1.1F;
        }, MemoryModuleType.NEAREST_VISIBLE_PLAYER, true)), Pair.of(3, BabyFollowAdult.create(HappyGhastAi.ADULT_FOLLOW_RANGE, (livingentity) -> {
            return 1.1F;
        }, MemoryModuleType.NEAREST_VISIBLE_ADULT, true)), Pair.of(4, new RunOne(ImmutableList.of(Pair.of(RandomStroll.fly(1.0F), 1), Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 1))))));
    }
