...
public record ProfilePublicKey(ProfilePublicKey.Data data) {
    ...
    public static ProfilePublicKey createValidated(SignatureValidator validator, UUID profileId, ProfilePublicKey.Data data) throws ProfilePublicKey.ValidationException {
        if (!data.validateSignature(validator, profileId)) {
            throw new ProfilePublicKey.ValidationException(ProfilePublicKey.INVALID_SIGNATURE);
        } else {
    ...
    public static record Data(Instant expiresAt, PublicKey key, byte[] keySignature) {
        ...
        private byte[] signedPayload(UUID profileId) {
            ...
            bytebuffer.putLong(profileId.getMostSignificantBits()).putLong(profileId.getLeastSignificantBits()).putLong(this.expiresAt.toEpochMilli()).put(abyte);
            return abyte1;
        }
        ...
        public boolean equals(Object o) {
            if (!(o instanceof ProfilePublicKey.Data profilepublickey_data)) {
                return false;
            } else {
