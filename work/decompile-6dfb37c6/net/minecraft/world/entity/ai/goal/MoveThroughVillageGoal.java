        ...
        if (!GoalUtils.hasGroundPathNavigation(this.mob)) {
            ...
            if (this.onlyAtNight && this.mob.level().isBrightOutside()) {
                ...
                if (!serverlevel.isCloseToVillage(blockpos, 6)) {
                    ...
                    Vec3 vec3 = LandRandomPos.getPos(this.mob, 15, 7, (blockpos1) -> {
                        if (!serverlevel.isVillage(blockpos1)) {
                            return Double.NEGATIVE_INFINITY;
                        } else {
                            Optional<BlockPos> optional = serverlevel.getPoiManager().find((holder) -> {
                                return holder.is(PoiTypeTags.VILLAGE);
                            }, this::hasNotVisited, blockpos1, 10, PoiManager.Occupancy.IS_OCCUPIED);
