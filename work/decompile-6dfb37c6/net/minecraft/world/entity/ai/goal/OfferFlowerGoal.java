...
public class OfferFlowerGoal extends Goal {
    ...
    public boolean canUse() {
        if (!this.golem.level().isBrightOutside()) {
            ...
            this.entity = getServerLevel((Entity) this.golem).getNearestEntity(EntityTypeTags.CANDIDATE_FOR_IRON_GOLEM_GIFT, OfferFlowerGoal.OFFER_TARGET_CONTEXT, this.golem, this.golem.getX(), this.golem.getY(), this.golem.getZ(), this.getGolemBoundingBox());
    ...
    public void stop() {
        ...
        if (this.tick == 0) {
            ...
            if (livingentity instanceof Mob) {
                ...
                if (mob.getType().is(EntityTypeTags.ACCEPTS_IRON_GOLEM_GIFT) && mob.getItemBySlot(CopperGolem.EQUIPMENT_SLOT_ANTENNA).isEmpty() && this.getGolemBoundingBox().intersects(mob.getBoundingBox())) {
                    mob.setItemSlot(CopperGolem.EQUIPMENT_SLOT_ANTENNA, OfferFlowerGoal.OFFER_ITEM.getDefaultInstance());
