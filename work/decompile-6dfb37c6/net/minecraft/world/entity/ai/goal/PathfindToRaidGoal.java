...
public class PathfindToRaidGoal<T extends Raider> extends Goal {
    ...
    private void recruitNearby(Raid raid) {
        if (raid.isActive()) {
            ...
            List<Raider> list = serverlevel.<Raider>getEntitiesOfClass(Raider.class, this.mob.getBoundingBox().inflate(16.0D), (raider) -> {
                return !raider.hasActiveRaid() && Raids.canJoinRaid(raider);
            });

            set.addAll(list);

            for (Raider raider : set) {
                raid.joinRaid(serverlevel, raid.getGroupsSpawned(), raider, (BlockPos) null, true);
            }
        }
