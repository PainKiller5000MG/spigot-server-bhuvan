        ...
        if (!this.llama.isLeashed() && !this.llama.inCaravan()) {
            ...
            for (Entity entity : list) {
                ...
                if (llama1.inCaravan() && !llama1.hasCaravanTail()) {
                    double d1 = this.llama.distanceToSqr((Entity) llama1);
            ...
            if (llama == null) {
                for (Entity entity1 : list) {
                    ...
                    if (llama2.isLeashed() && !llama2.hasCaravanTail()) {
                        double d2 = this.llama.distanceToSqr((Entity) llama2);
    ...
    private boolean firstIsLeashed(Llama currentMob, int counter) {
        if (counter > 8) {
            ...
            if (currentMob.getCaravanHead().isLeashed()) {
                ...
                Llama llama1 = currentMob.getCaravanHead();

                ++counter;
                return this.firstIsLeashed(llama1, counter);
            }
        } else {
            return false;
        }
