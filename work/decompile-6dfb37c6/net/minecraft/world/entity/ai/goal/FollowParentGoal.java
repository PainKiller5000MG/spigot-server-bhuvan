...
public class FollowParentGoal extends Goal {
    ...
    public boolean canUse() {
        if (this.animal.getAge() >= 0) {
            ...
            List<? extends Animal> list = this.animal.level().<Animal>getEntitiesOfClass(this.animal.getClass(), this.animal.getBoundingBox().inflate(8.0D, 4.0D, 8.0D));
            Animal animal = null;
            double d0 = Double.MAX_VALUE;

            for (Animal animal1 : list) {
                if (animal1.getAge() >= 0) {
                    double d1 = this.animal.distanceToSqr((Entity) animal1);
