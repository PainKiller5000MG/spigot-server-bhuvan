...
public class ResetUniversalAngerTargetGoal<T extends Mob & NeutralMob> extends Goal {
    ...
    public boolean canUse() {
        return (Boolean) getServerLevel((Entity) this.mob).getGameRules().get(GameRules.UNIVERSAL_ANGER) && this.wasHurtByPlayer();
    }
    ...
    public void start() {
        ...
        if (this.alertOthersOfSameType) {
            this.getNearbyMobsOfSameType().stream().filter((mob) -> {
                return mob != this.mob;
            }).map((mob) -> {
                return (NeutralMob) mob;
    ...
    private List<? extends Mob> getNearbyMobsOfSameType() {
        ...
        return this.mob.level().<Mob>getEntitiesOfClass(this.mob.getClass(), aabb, EntitySelector.NO_SPECTATORS);
    }
}
