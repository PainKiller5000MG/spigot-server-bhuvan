...
public class Sniffing<E extends Warden> extends Behavior<E> {
    ...
    protected void stop(ServerLevel level, E body, long timestamp) {
        ...
        Optional optional = body.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE);

        Objects.requireNonNull(body);
        optional.filter(body::canTargetEntity).ifPresent((livingentity) -> {
            if (body.closerThan(livingentity, 6.0D, 20.0D)) {
                body.increaseAngerAt(livingentity);
            }

            if (!body.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
                WardenAi.setDisturbanceLocation(body, livingentity.blockPosition());
            }
