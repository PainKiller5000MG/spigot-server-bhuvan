...
public class VillagerGoalPackages {
    ...
    public static ImmutableList<Pair<Integer, ? extends BehaviorControl<? super Villager>>> getRestPackage(Holder<VillagerProfession> profession, float speedModifier) {
        return ImmutableList.of(Pair.of(2, SetWalkTargetFromBlockMemory.create(MemoryModuleType.HOME, speedModifier, 1, 150, 1200)), Pair.of(3, ValidateNearbyPoi.create((holder1) -> {
            return holder1.is(PoiTypes.HOME);
        }, MemoryModuleType.HOME)), Pair.of(3, new SleepInBed()), Pair.of(5, new RunOne(ImmutableMap.of(MemoryModuleType.HOME, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(SetClosestHomeAsWalkTarget.create(speedModifier), 1), Pair.of(InsideBrownianWalk.create(speedModifier), 4), Pair.of(GoToClosestVillage.create(speedModifier, 4), 2), Pair.of(new DoNothing(20, 40), 2)))), getMinimalLookBehavior(), Pair.of(99, UpdateActivityFromSchedule.create()));
    }
    ...
    public static ImmutableList<Pair<Integer, ? extends BehaviorControl<? super Villager>>> getPanicPackage(Holder<VillagerProfession> profession, float speedModifier) {
        float f1 = speedModifier * 1.5F;

        return ImmutableList.of(Pair.of(0, VillagerCalmDown.create()), Pair.of(1, SetWalkTargetAwayFrom.entity(MemoryModuleType.NEAREST_HOSTILE, f1, 6, false)), Pair.of(1, SetWalkTargetAwayFrom.entity(MemoryModuleType.HURT_BY_ENTITY, f1, 6, false)), Pair.of(3, VillageBoundRandomStroll.create(f1, 2, 2)), getMinimalLookBehavior());
    }
    ...
    private static boolean raidExistsAndActive(ServerLevel level, LivingEntity body) {
        Raid raid = level.getRaidAt(body.blockPosition());
