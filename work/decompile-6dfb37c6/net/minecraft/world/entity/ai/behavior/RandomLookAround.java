...
public class RandomLookAround extends Behavior<Mob> {
    ...
    protected void start(ServerLevel level, Mob body, long timestamp) {
        ...
        float f1 = Mth.wrapDegrees(body.getYRot() + 2.0F * randomsource.nextFloat() * this.maxYaw - this.maxYaw);
        Vec3 vec3 = Vec3.directionFromRotation(f, f1);

        body.getBrain().setMemory(MemoryModuleType.LOOK_TARGET, new BlockPosTracker(body.getEyePosition().add(vec3)));
        body.getBrain().setMemory(MemoryModuleType.GAZE_COOLDOWN_TICKS, this.interval.sample(randomsource));
    }
}
