        ...
        return BehaviorBuilder.create((behaviorbuilder_instance) -> {
            return behaviorbuilder_instance.group(behaviorbuilder_instance.registered(MemoryModuleType.WALK_TARGET), behaviorbuilder_instance.registered(MemoryModuleType.LOOK_TARGET), behaviorbuilder_instance.present(MemoryModuleType.MEETING_POINT), behaviorbuilder_instance.present(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES), behaviorbuilder_instance.absent(MemoryModuleType.INTERACTION_TARGET)).apply(behaviorbuilder_instance, (memoryaccessor, memoryaccessor1, memoryaccessor2, memoryaccessor3, memoryaccessor4) -> {
                return (serverlevel, livingentity, i) -> {
                    ...
                    if (serverlevel.getRandom().nextInt(100) == 0 && serverlevel.dimension() == globalpos.dimension() && globalpos.pos().closerToCenterThan(livingentity.position(), 4.0D) && nearestvisiblelivingentities.contains((livingentity1) -> {
                        return EntityType.VILLAGER.equals(livingentity1.getType());
                    })) {
                        nearestvisiblelivingentities.findClosest((livingentity1) -> {
                            return EntityType.VILLAGER.equals(livingentity1.getType()) && livingentity1.distanceToSqr((Entity) livingentity) <= 32.0D;
