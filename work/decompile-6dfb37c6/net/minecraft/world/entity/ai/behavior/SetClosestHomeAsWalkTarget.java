...
public class SetClosestHomeAsWalkTarget {
    ...
    public static BehaviorControl<PathfinderMob> create(float speedModifier) {
        ...
        return BehaviorBuilder.create((behaviorbuilder_instance) -> {
            return behaviorbuilder_instance.group(behaviorbuilder_instance.absent(MemoryModuleType.WALK_TARGET), behaviorbuilder_instance.absent(MemoryModuleType.HOME)).apply(behaviorbuilder_instance, (memoryaccessor, memoryaccessor1) -> {
                return (serverlevel, pathfindermob, i) -> {
                    if (serverlevel.getGameTime() - mutablelong.longValue() < 20L) {
                        return false;
                    } else {
                        PoiManager poimanager = serverlevel.getPoiManager();
                        Optional<BlockPos> optional = poimanager.findClosest((holder) -> {
                            return holder.is(PoiTypes.HOME);
                        }, pathfindermob.blockPosition(), 48, PoiManager.Occupancy.ANY);

                        if (!optional.isEmpty() && ((BlockPos) optional.get()).distSqr(pathfindermob.blockPosition()) > 4.0D) {
                            MutableInt mutableint = new MutableInt(0);

                            mutablelong.setValue(serverlevel.getGameTime() + (long) serverlevel.getRandom().nextInt(20));
                            Predicate<BlockPos> predicate = (blockpos) -> {
                                long j = blockpos.asLong();
                                ...
                                if (long2longmap.containsKey(j)) {
                                    ...
                                    long2longmap.put(j, mutablelong.longValue() + 40L);
                                    return true;
                                }
                            };
                            Set<Pair<Holder<PoiType>, BlockPos>> set = (Set) poimanager.findAllWithType((holder) -> {
                                return holder.is(PoiTypes.HOME);
                            }, predicate, pathfindermob.blockPosition(), 48, PoiManager.Occupancy.ANY).collect(Collectors.toSet());
                            ...
                            if (path != null && path.canReach()) {
                                ...
                                long2longmap.long2LongEntrySet().removeIf((entry) -> {
                                    return entry.getLongValue() < mutablelong.longValue();
                                });
                            }
