...
public class TryLaySpawnOnWaterNearLand {
    ...
    public static BehaviorControl<LivingEntity> create(Block spawnBlock) {
        return BehaviorBuilder.create((behaviorbuilder_instance) -> {
            return behaviorbuilder_instance.group(behaviorbuilder_instance.absent(MemoryModuleType.ATTACK_TARGET), behaviorbuilder_instance.present(MemoryModuleType.WALK_TARGET), behaviorbuilder_instance.present(MemoryModuleType.IS_PREGNANT)).apply(behaviorbuilder_instance, (memoryaccessor, memoryaccessor1, memoryaccessor2) -> {
                        ...
                        for (Direction direction : Direction.Plane.HORIZONTAL) {
                            ...
                            if (serverlevel.getBlockState(blockpos1).getCollisionShape(serverlevel, blockpos1).getFaceShape(Direction.UP).isEmpty() && serverlevel.getFluidState(blockpos1).is(Fluids.WATER)) {
                                ...
                                if (serverlevel.getBlockState(blockpos2).isAir()) {
                                    ...
                                    serverlevel.playSound((Entity) null, (Entity) livingentity, SoundEvents.FROG_LAY_SPAWN, SoundSource.BLOCKS, 1.0F, 1.0F);
