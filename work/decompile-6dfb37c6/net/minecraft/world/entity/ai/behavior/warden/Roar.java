...
public class Roar extends Behavior<Warden> {
    ...
    protected void start(ServerLevel level, Warden body, long timestamp) {
        ...
        LivingEntity livingentity = (LivingEntity) body.getBrain().getMemory(MemoryModuleType.ROAR_TARGET).get();

        BehaviorUtils.lookAtEntity(body, livingentity);
        body.setPose(Pose.ROARING);
        body.increaseAngerAt(livingentity, 20, false);
    }
    ...
    protected void stop(ServerLevel level, Warden body, long timestamp) {
        ...
        Optional optional = body.getBrain().getMemory(MemoryModuleType.ROAR_TARGET);

        Objects.requireNonNull(body);
        optional.ifPresent(body::setAttackTarget);
