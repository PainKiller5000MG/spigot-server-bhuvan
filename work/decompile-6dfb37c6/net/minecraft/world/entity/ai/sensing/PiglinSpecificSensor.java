...
public class PiglinSpecificSensor extends Sensor<LivingEntity> {
    ...
    protected void doTick(ServerLevel level, LivingEntity body) {
        ...
        for (LivingEntity livingentity1 : nearestvisiblelivingentities.findAll((livingentity2) -> {
            return true;
        })) {
            if (livingentity1 instanceof Hoglin hoglin) {
                ...
                if (optional5.isEmpty() && !PiglinAi.isWearingSafeArmor(player) && body.canAttack(livingentity1)) {
    ...
    private static Optional<BlockPos> findNearestRepellent(ServerLevel level, LivingEntity body) {
        return BlockPos.findClosestMatch(body.blockPosition(), 8, 4, (blockpos) -> {
            return isValidRepellent(level, blockpos);
        });
    }
