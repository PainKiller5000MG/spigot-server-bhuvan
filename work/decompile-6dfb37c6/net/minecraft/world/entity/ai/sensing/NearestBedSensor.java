...
public class NearestBedSensor extends Sensor<Mob> {
    ...
    protected void doTick(ServerLevel level, Mob body) {
        if (body.isBaby()) {
            ...
            }, predicate, body.blockPosition(), 48, PoiManager.Occupancy.ANY).collect(Collectors.toSet());
            Path path = AcquirePoi.findPathToPois(body, set);
            ...
            if (path != null && path.canReach()) {
                ...
                if (optional.isPresent()) {
                    body.getBrain().setMemory(MemoryModuleType.NEAREST_BED, blockpos);
                }
            } else if (this.triedCount < 5) {
