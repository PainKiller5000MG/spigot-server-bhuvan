...
public class SpawnPlacements {
    ...
    private static <T extends Mob> void register(EntityType<T> type, SpawnPlacementType placementType, Heightmap.Types heightmap, SpawnPlacements.SpawnPredicate<T> spawnPredicate) {
        SpawnPlacements.Data spawnplacements_data = (SpawnPlacements.Data) SpawnPlacements.DATA_BY_TYPE.put(type, new SpawnPlacements.Data(heightmap, placementType, spawnPredicate));

        if (spawnplacements_data != null) {
    ...
    public static SpawnPlacementType getPlacementType(EntityType<?> type) {
        SpawnPlacements.Data spawnplacements_data = (SpawnPlacements.Data) SpawnPlacements.DATA_BY_TYPE.get(type);

        return spawnplacements_data == null ? SpawnPlacementTypes.NO_RESTRICTIONS : spawnplacements_data.placement;
    }
    ...
    public static <T extends Entity> boolean checkSpawnRules(EntityType<T> type, ServerLevelAccessor level, EntitySpawnReason spawnReason, BlockPos pos, RandomSource random) {
        ...
        return spawnplacements_data == null || spawnplacements_data.predicate.test(type, level, spawnReason, pos, random);
    }
    ...
    private static record Data(Heightmap.Types heightMap, SpawnPlacementType placement, SpawnPlacements.SpawnPredicate<?> predicate) {
