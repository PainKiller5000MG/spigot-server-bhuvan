...
public class EuclideanGameEventListenerRegistry implements GameEventListenerRegistry {
    ...
    private static void sendDebugInfo(ServerLevel level, GameEventListener listener) {
        if (level.debugSynchronizers().hasAnySubscriberFor(DebugSubscriptions.GAME_EVENT_LISTENERS)) {
            ...
            PositionSource positionsource = listener.getListenerSource();
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (this.listenersToRemove.remove(gameeventlistener)) {
                    ...
                    Optional<Vec3> optional = getPostableListenerPosition(this.level, sourcePosition, gameeventlistener);

                    if (optional.isPresent()) {
                        action.visit(gameeventlistener, (Vec3) optional.get());
                        flag = true;
                    }
                }
            }
    ...
    private static Optional<Vec3> getPostableListenerPosition(ServerLevel level, Vec3 sourcePosition, GameEventListener listener) {
        ...
        if (optional.isEmpty()) {
            ...
            double d0 = BlockPos.containing((Position) optional.get()).distSqr(BlockPos.containing(sourcePosition));
