...
public class VibrationSelector {
    ...
    public static final Codec<VibrationSelector> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(VibrationInfo.CODEC.lenientOptionalFieldOf("event").forGetter((vibrationselector) -> {
    ...
    public VibrationSelector(Optional<VibrationInfo> currentVibration, long tick) {
        this.currentVibrationData = currentVibration.map((vibrationinfo) -> {
            return Pair.of(vibrationinfo, tick);
        });
    }
    ...
    private boolean shouldReplaceVibration(VibrationInfo newVibration, long tickTime) {
        if (this.currentVibrationData.isEmpty()) {
            ...
            if (tickTime != j) {
                ...
                return newVibration.distance() < vibrationinfo1.distance() ? true : (newVibration.distance() > vibrationinfo1.distance() ? false : VibrationSystem.getGameEventFrequency(newVibration.gameEvent()) > VibrationSystem.getGameEventFrequency(vibrationinfo1.gameEvent()));
            }
        }
    }

    public Optional<VibrationInfo> chosenCandidate(long time) {
        return this.currentVibrationData.isEmpty() ? Optional.empty() : ((Long) ((Pair) this.currentVibrationData.get()).getRight() < time ? Optional.of((VibrationInfo) ((Pair) this.currentVibrationData.get()).getLeft()) : Optional.empty());
    }
