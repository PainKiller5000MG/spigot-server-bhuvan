...
public class CrafterBlockEntity extends RandomizableContainerBlockEntity implements CraftingContainer {
    ...
    protected Component getDefaultName() {
        return CrafterBlockEntity.DEFAULT_NAME;
    }
    ...
    private boolean smallerStackExist(int baseSize, ItemStack baseItem, int baseSlot) {
        for (int k = baseSlot + 1; k < 9; ++k) {
            if (!this.isSlotDisabled(k)) {
                ItemStack itemstack1 = this.getItem(k);

                if (itemstack1.isEmpty() || itemstack1.getCount() < baseSize && ItemStack.isSameItemSameComponents(itemstack1, baseItem)) {
                    return true;
                }
            }
        }
    ...
    protected void loadAdditional(ValueInput input) {
        ...
        input.getIntArray("disabled_slots").ifPresent((aint) -> {
            for (int j : aint) {
    ...
    public static void serverTick(Level level, BlockPos blockPos, BlockState blockState, CrafterBlockEntity entity) {
        int i = entity.craftingTicksRemaining - 1;

        if (i >= 0) {
            entity.craftingTicksRemaining = i;
            if (i == 0) {
