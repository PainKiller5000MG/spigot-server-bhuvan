...
public record StorageValue(Identifier storage, NbtPathArgument.NbtPath path) implements NumberProvider {
    ...
    public static final MapCodec<StorageValue> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(Identifier.CODEC.fieldOf("storage").forGetter(StorageValue::storage), NbtPathArgument.NbtPath.CODEC.fieldOf("path").forGetter(StorageValue::path)).apply(instance, StorageValue::new);
    });
    ...
    private Number getNumericTag(LootContext context, Number _default) {
        ...
        try {
            ...
            if (list.size() == 1) {
                ...
                if (object instanceof NumericTag) {
