...
public class SetContainerLootTable extends LootItemConditionalFunction {
    ...
    public static final MapCodec<SetContainerLootTable> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return commonFields(instance).and(instance.group(LootTable.KEY_CODEC.fieldOf("name").forGetter((setcontainerloottable) -> {
            return setcontainerloottable.name;
        }), Codec.LONG.optionalFieldOf("seed", 0L).forGetter((setcontainerloottable) -> {
            return setcontainerloottable.seed;
        }), BuiltInRegistries.BLOCK_ENTITY_TYPE.holderByNameCodec().fieldOf("type").forGetter((setcontainerloottable) -> {
            return setcontainerloottable.type;
    ...
    public static LootItemConditionalFunction.Builder<?> withLootTable(BlockEntityType<?> type, ResourceKey<LootTable> value) {
        return simpleBuilder((list) -> {
            return new SetContainerLootTable(list, value, 0L, type.builtInRegistryHolder());
        });
    }
    ...
    public static LootItemConditionalFunction.Builder<?> withLootTable(BlockEntityType<?> type, ResourceKey<LootTable> value, long seed) {
        return simpleBuilder((list) -> {
            return new SetContainerLootTable(list, value, seed, type.builtInRegistryHolder());
        });
    }
}
