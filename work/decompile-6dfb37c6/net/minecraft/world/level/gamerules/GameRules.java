...
public class GameRules {
    ...
    public static final GameRule<Integer> MAX_MINECART_SPEED = registerInteger("max_minecart_speed", GameRuleCategory.MISC, 8, 1, 1000, FeatureFlagSet.of(FeatureFlags.MINECART_IMPROVEMENTS));
    ...
    public static Codec<GameRules> codec(FeatureFlagSet enabledFeatures) {
        return GameRuleMap.CODEC.xmap((gamerulemap) -> {
            return new GameRules(enabledFeatures, gamerulemap);
        }, (gamerules) -> {
    ...
    public GameRules(FeatureFlagSet enabledFeatures, GameRuleMap map) {
        ...
        gamerulemap1.setFromIf(map, gamerulemap2::has);
    }
    ...
    public <T> T get(GameRule<T> gameRule) {
        T t0 = (T) this.rules.get(gameRule);
    ...
    private static GameRule<Boolean> registerBoolean(String id, GameRuleCategory category, boolean defaultValue) {
        return register(id, category, GameRuleType.BOOL, BoolArgumentType.bool(), Codec.BOOL, defaultValue, FeatureFlagSet.of(), GameRuleTypeVisitor::visitBoolean, (obool) -> {
            return obool ? 1 : 0;
        });
    }
    ...
    private static GameRule<Integer> registerInteger(String id, GameRuleCategory category, int defaultValue, int min, int max, FeatureFlagSet requiredFeatures) {
        return register(id, category, GameRuleType.INT, IntegerArgumentType.integer(min, max), Codec.intRange(min, max), defaultValue, requiredFeatures, GameRuleTypeVisitor::visitInteger, (integer) -> {
            return integer;
        });
    }

    private static <T> GameRule<T> register(String id, GameRuleCategory category, GameRuleType typeHint, ArgumentType<T> argumentType, Codec<T> codec, T defaultValue, FeatureFlagSet requiredFeatures, GameRules.VisitorCaller<T> visitorCaller, ToIntFunction<T> commandResultFunction) {
        return (GameRule) Registry.register(BuiltInRegistries.GAME_RULE, id, new GameRule(category, typeHint, argumentType, visitorCaller, codec, commandResultFunction, defaultValue, requiredFeatures));
    }
