...
public class MapColor {
    ...
    public static final @Nullable MapColor[] MATERIAL_COLORS = new MapColor[64];
    ...
    private MapColor(int id, int col) {
        if (id >= 0 && id <= 63) {
            this.id = id;
            this.col = col;
            MapColor.MATERIAL_COLORS[id] = this;
        } else {
    ...
    public int calculateARGBColor(MapColor.Brightness brightness) {
        return this == MapColor.NONE ? 0 : ARGB.scaleRGB(ARGB.opaque(this.col), brightness.modifier);
    }

    public static MapColor byId(int id) {
        Preconditions.checkPositionIndex(id, MapColor.MATERIAL_COLORS.length, "material id");
        return byIdUnsafe(id);
    }

    private static MapColor byIdUnsafe(int id) {
        MapColor mapcolor = MapColor.MATERIAL_COLORS[id];

        return mapcolor != null ? mapcolor : MapColor.NONE;
    }
    ...
    public static enum Brightness {
        ...
        private static final MapColor.Brightness[] VALUES = new MapColor.Brightness[]{MapColor.Brightness.LOW, MapColor.Brightness.NORMAL, MapColor.Brightness.HIGH, MapColor.Brightness.LOWEST};
        public final int id;
        ...
        public static MapColor.Brightness byId(int id) {
            Preconditions.checkPositionIndex(id, MapColor.Brightness.VALUES.length, "brightness id");
            return byIdUnsafe(id);
        }

        private static MapColor.Brightness byIdUnsafe(int id) {
            return MapColor.Brightness.VALUES[id];
        }
    }
}
