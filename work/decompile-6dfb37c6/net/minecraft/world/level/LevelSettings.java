...
public final class LevelSettings {
    ...
    public static LevelSettings parse(Dynamic<?> input, WorldDataConfiguration loadConfig) {
        ...
        String s = input.get("LevelName").asString("");
        boolean flag = input.get("hardcore").asBoolean(false);
        Difficulty difficulty = (Difficulty) input.get("Difficulty").asNumber().map((number) -> {
            return Difficulty.byId(number.byteValue());
        }).result().orElse(Difficulty.NORMAL);
        boolean flag1 = input.get("allowCommands").asBoolean(gametype == GameType.CREATIVE);
        DataResult dataresult = GameRules.codec(loadConfig.enabledFeatures()).parse(input.get("game_rules").orElseEmptyMap());
        Logger logger = LevelSettings.LOGGER;

        Objects.requireNonNull(logger);
        return new LevelSettings(s, gametype, flag, difficulty, flag1, (GameRules) dataresult.resultOrPartial(logger::warn).orElseThrow(), loadConfig);
    }
