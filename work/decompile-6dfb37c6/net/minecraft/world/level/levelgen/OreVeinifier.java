...
public final class OreVeinifier {
    ...
    protected static NoiseChunk.BlockStateFiller create(DensityFunction veinToggle, DensityFunction veinRidged, DensityFunction veinGap, PositionalRandomFactory oreVeinsPositionalRandomFactory) {
        ...
        return (densityfunction_functioncontext) -> {
            double d0 = veinToggle.compute(densityfunction_functioncontext);
            ...
            if (k >= 0 && j >= 0) {
                ...
                if (d1 + d2 < (double) 0.4F) {
                    ...
                    RandomSource randomsource = oreVeinsPositionalRandomFactory.at(densityfunction_functioncontext.blockX(), i, densityfunction_functioncontext.blockZ());

                    if (randomsource.nextFloat() > 0.7F) {
                        return blockstate;
                    } else if (veinRidged.compute(densityfunction_functioncontext) >= 0.0D) {
                        return blockstate;
                    } else {
                        double d3 = Mth.clampedMap(d1, (double) 0.4F, (double) 0.6F, (double) 0.1F, (double) 0.3F);

                        return (double) randomsource.nextFloat() < d3 && veinGap.compute(densityfunction_functioncontext) > (double) -0.3F ? (randomsource.nextFloat() < 0.02F ? oreveinifier_veintype.rawOreBlock : oreveinifier_veintype.ore) : (SharedConstants.DEBUG_ORE_VEINS ? Blocks.OAK_BUTTON.defaultBlockState() : oreveinifier_veintype.filler);
                    }
                }
            } else {
                return blockstate;
