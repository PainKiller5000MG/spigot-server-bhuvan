...
public interface VerticalAnchor {
    ...
    private static Either<VerticalAnchor.Absolute, Either<VerticalAnchor.AboveBottom, VerticalAnchor.BelowTop>> split(VerticalAnchor anchor) {
        return anchor instanceof VerticalAnchor.Absolute ? Either.left((VerticalAnchor.Absolute) anchor) : Either.right(anchor instanceof VerticalAnchor.AboveBottom ? Either.left((VerticalAnchor.AboveBottom) anchor) : Either.right((VerticalAnchor.BelowTop) anchor));
    }

    int resolveY(WorldGenerationContext heightAccessor);
