...
public record WorldGenSettings(WorldOptions options, WorldDimensions dimensions) {
    ...
    public static final Codec<WorldGenSettings> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(WorldOptions.CODEC.forGetter(WorldGenSettings::options), WorldDimensions.CODEC.forGetter(WorldGenSettings::dimensions)).apply(instance, instance.stable(WorldGenSettings::new));
    });

    public static <T> DataResult<T> encode(DynamicOps<T> ops, WorldOptions options, WorldDimensions dimensions) {
        return WorldGenSettings.CODEC.encodeStart(ops, new WorldGenSettings(options, dimensions));
    }
