...
public class HasSturdyFacePredicate implements BlockPredicate {
    ...
    public static final MapCodec<HasSturdyFacePredicate> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        ...
        }), Direction.CODEC.fieldOf("direction").forGetter((hassturdyfacepredicate) -> {
    ...
    public boolean test(WorldGenLevel level, BlockPos origin) {
        BlockPos blockpos1 = origin.offset(this.offset);

        return level.getBlockState(blockpos1).isFaceSturdy(level, blockpos1, this.direction);
    }
