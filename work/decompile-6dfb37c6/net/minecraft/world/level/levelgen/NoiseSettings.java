...
public record NoiseSettings(int minY, int height, int noiseSizeHorizontal, int noiseSizeVertical) {
    ...
    private static DataResult<NoiseSettings> guardY(NoiseSettings dimensionType) {
        return dimensionType.minY() + dimensionType.height() > DimensionType.MAX_Y + 1 ? DataResult.error(() -> {
            return "min_y + height cannot be higher than: " + (DimensionType.MAX_Y + 1);
        }) : (dimensionType.height() % 16 != 0 ? DataResult.error(() -> {
            return "height has to be a multiple of 16";
        }) : (dimensionType.minY() % 16 != 0 ? DataResult.error(() -> {
            return "min_y has to be a multiple of 16";
        }) : DataResult.success(dimensionType)));
    }
    ...
    public NoiseSettings clampToHeightAccessor(LevelHeightAccessor heightAccessor) {
        int i = Math.max(this.minY, heightAccessor.getMinY());
        int j = Math.min(this.minY + this.height, heightAccessor.getMaxY() + 1) - i;
