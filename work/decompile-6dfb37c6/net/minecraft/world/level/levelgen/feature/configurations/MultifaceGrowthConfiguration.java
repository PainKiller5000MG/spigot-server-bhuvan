...
public class MultifaceGrowthConfiguration implements FeatureConfiguration {
    ...
    public static final Codec<MultifaceGrowthConfiguration> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(BuiltInRegistries.BLOCK.byNameCodec().fieldOf("block").flatXmap(MultifaceGrowthConfiguration::apply, DataResult::success).orElse((MultifaceSpreadeableBlock) Blocks.GLOW_LICHEN).forGetter((multifacegrowthconfiguration) -> {
    ...
    private static DataResult<MultifaceSpreadeableBlock> apply(Block block) {
        ...
        if (block instanceof MultifaceSpreadeableBlock multifacespreadeableblock) {
            dataresult = DataResult.success(multifacespreadeableblock);
        } else {
    ...
    public MultifaceGrowthConfiguration(MultifaceSpreadeableBlock placeBlock, int searchRange, boolean canPlaceOnFloor, boolean canPlaceOnCeiling, boolean canPlaceOnWall, float chanceOfSpreading, HolderSet<Block> canBePlacedOn) {
        ...
        if (canPlaceOnWall) {
            Direction.Plane direction_plane = Direction.Plane.HORIZONTAL;
            ObjectArrayList objectarraylist = this.validDirections;

            Objects.requireNonNull(this.validDirections);
            direction_plane.forEach(objectarraylist::add);
        }

    }

    public List<Direction> getShuffledDirectionsExcept(RandomSource random, Direction excludeDirection) {
        return Util.toShuffledList(this.validDirections.stream().filter((direction1) -> {
            return direction1 != excludeDirection;
        }), random);
    }
