...
public class VoidStartPlatformFeature extends Feature<NoneFeatureConfiguration> {
    ...
    private static final ChunkPos PLATFORM_ORIGIN_CHUNK = new ChunkPos(VoidStartPlatformFeature.PLATFORM_OFFSET);
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (checkerboardDistance(chunkpos.x, chunkpos.z, VoidStartPlatformFeature.PLATFORM_ORIGIN_CHUNK.x, VoidStartPlatformFeature.PLATFORM_ORIGIN_CHUNK.z) > 1) {
            return true;
        } else {
            BlockPos blockpos = VoidStartPlatformFeature.PLATFORM_OFFSET.atY(context.origin().getY() + VoidStartPlatformFeature.PLATFORM_OFFSET.getY());
