...
public class StructureTemplatePool {
    ...
    public static final Codec<StructureTemplatePool> DIRECT_CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(Codec.lazyInitialized(StructureTemplatePool.CODEC_REFERENCE).fieldOf("fallback").forGetter(StructureTemplatePool::getFallback), Codec.mapPair(StructurePoolElement.CODEC.fieldOf("element"), Codec.intRange(1, 150).fieldOf("weight")).codec().listOf().fieldOf("elements").forGetter((structuretemplatepool) -> {
    ...
    public int getMaxSize(StructureTemplateManager manager) {
        if (this.maxSize == Integer.MIN_VALUE) {
            this.maxSize = this.templates.stream().filter((structurepoolelement) -> {
                ...
                return structurepoolelement.getBoundingBox(manager, BlockPos.ZERO, Rotation.NONE).getYSpan();
            }).max().orElse(0);
        }
    ...
    public static enum Projection implements StringRepresentable {
        ...
        TERRAIN_MATCHING("terrain_matching", ImmutableList.of(new GravityProcessor(Heightmap.Types.WORLD_SURFACE_WG, -1))), RIGID("rigid", ImmutableList.of());

        public static final StringRepresentable.EnumCodec<StructureTemplatePool.Projection> CODEC = StringRepresentable.<StructureTemplatePool.Projection>fromEnum(StructureTemplatePool.Projection::values);
