...
public interface PieceGeneratorSupplier<C extends FeatureConfiguration> {
    ...
    static <C extends FeatureConfiguration> PieceGeneratorSupplier<C> simple(Predicate<PieceGeneratorSupplier.Context<C>> check, PieceGenerator<C> generator) {
        ...
        return (piecegeneratorsupplier_context) -> {
            return check.test(piecegeneratorsupplier_context) ? optional : Optional.empty();
        };
    }

    static <C extends FeatureConfiguration> Predicate<PieceGeneratorSupplier.Context<C>> checkForBiomeOnTop(Heightmap.Types type) {
        return (piecegeneratorsupplier_context) -> {
            return piecegeneratorsupplier_context.validBiomeOnTop(type);
        };
    }
    ...
    public static record Context<C extends FeatureConfiguration>(ChunkGenerator chunkGenerator, BiomeSource biomeSource, RandomState randomState, long seed, ChunkPos chunkPos, C config, LevelHeightAccessor heightAccessor, Predicate<Holder<Biome>> validBiome, StructureTemplateManager structureTemplateManager, RegistryAccess registryAccess) {
        ...
        public boolean validBiomeOnTop(Heightmap.Types type) {
            ...
            int k = this.chunkGenerator.getFirstOccupiedHeight(i, j, type, this.heightAccessor, this.randomState);
