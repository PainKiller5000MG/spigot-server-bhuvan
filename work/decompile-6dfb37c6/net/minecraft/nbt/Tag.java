...
public sealed interface Tag permits CompoundTag, CollectionTag, PrimitiveTag, EndTag {
    ...
    default void acceptAsRoot(StreamTagVisitor output) {
        StreamTagVisitor.ValueResult streamtagvisitor_valueresult = output.visitRootEntry(this.getType());

        if (streamtagvisitor_valueresult == StreamTagVisitor.ValueResult.CONTINUE) {
            this.accept(output);
        }
