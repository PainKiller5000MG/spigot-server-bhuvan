...
public class DustParticleOptions extends ScalableParticleOptionsBase {
    ...
    public static final MapCodec<DustParticleOptions> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(ExtraCodecs.RGB_COLOR_CODEC.fieldOf("color").forGetter((dustparticleoptions) -> {
            return dustparticleoptions.color;
        }), DustParticleOptions.SCALE.fieldOf("scale").forGetter(ScalableParticleOptionsBase::getScale)).apply(instance, DustParticleOptions::new);
    });
    public static final StreamCodec<RegistryFriendlyByteBuf, DustParticleOptions> STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.INT, (dustparticleoptions) -> {
        return dustparticleoptions.color;
