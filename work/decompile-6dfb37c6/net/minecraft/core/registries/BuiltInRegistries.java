...
public class BuiltInRegistries {
    ...
    private static final WritableRegistry<WritableRegistry<?>> WRITABLE_REGISTRY = new MappedRegistry<WritableRegistry<?>>(ResourceKey.createRegistryKey(Registries.ROOT_REGISTRY_NAME), Lifecycle.stable());
    ...
    public static final Registry<Identifier> CUSTOM_STAT = registerSimple(Registries.CUSTOM_STAT, (registry) -> {
        return Stats.JUMP;
    });
    ...
    public static final DefaultedRegistry<VillagerProfession> VILLAGER_PROFESSION = registerDefaulted(Registries.VILLAGER_PROFESSION, "none", VillagerProfession::bootstrap);
    ...
    public static final Registry<MapCodec<? extends SurfaceRules.ConditionSource>> MATERIAL_CONDITION = registerSimple(Registries.MATERIAL_CONDITION, SurfaceRules.ConditionSource::bootstrap);
    public static final Registry<MapCodec<? extends SurfaceRules.RuleSource>> MATERIAL_RULE = registerSimple(Registries.MATERIAL_RULE, SurfaceRules.RuleSource::bootstrap);
    public static final Registry<MapCodec<? extends DensityFunction>> DENSITY_FUNCTION_TYPE = registerSimple(Registries.DENSITY_FUNCTION_TYPE, DensityFunctions::bootstrap);
    ...
    public static final Registry<MapCodec<? extends PoolAliasBinding>> POOL_ALIAS_BINDING_TYPE = registerSimple(Registries.POOL_ALIAS_BINDING, PoolAliasBindings::bootstrap);
    ...
    public static final Registry<MapCodec<? extends EntitySubPredicate>> ENTITY_SUB_PREDICATE_TYPE = registerSimple(Registries.ENTITY_SUB_PREDICATE_TYPE, EntitySubPredicates::bootstrap);
    public static final Registry<DataComponentPredicate.Type<?>> DATA_COMPONENT_PREDICATE_TYPE = registerSimple(Registries.DATA_COMPONENT_PREDICATE_TYPE, DataComponentPredicates::bootstrap);
    ...
    public static final Registry<DataComponentType<?>> ENCHANTMENT_EFFECT_COMPONENT_TYPE = registerSimple(Registries.ENCHANTMENT_EFFECT_COMPONENT_TYPE, EnchantmentEffectComponents::bootstrap);
    public static final Registry<MapCodec<? extends LevelBasedValue>> ENCHANTMENT_LEVEL_BASED_VALUE_TYPE = registerSimple(Registries.ENCHANTMENT_LEVEL_BASED_VALUE_TYPE, LevelBasedValue::bootstrap);
    public static final Registry<MapCodec<? extends EnchantmentEntityEffect>> ENCHANTMENT_ENTITY_EFFECT_TYPE = registerSimple(Registries.ENCHANTMENT_ENTITY_EFFECT_TYPE, EnchantmentEntityEffect::bootstrap);
    public static final Registry<MapCodec<? extends EnchantmentLocationBasedEffect>> ENCHANTMENT_LOCATION_BASED_EFFECT_TYPE = registerSimple(Registries.ENCHANTMENT_LOCATION_BASED_EFFECT_TYPE, EnchantmentLocationBasedEffect::bootstrap);
    public static final Registry<MapCodec<? extends EnchantmentValueEffect>> ENCHANTMENT_VALUE_EFFECT_TYPE = registerSimple(Registries.ENCHANTMENT_VALUE_EFFECT_TYPE, EnchantmentValueEffect::bootstrap);
    public static final Registry<MapCodec<? extends EnchantmentProvider>> ENCHANTMENT_PROVIDER_TYPE = registerSimple(Registries.ENCHANTMENT_PROVIDER_TYPE, EnchantmentProviderTypes::bootstrap);
    public static final Registry<ConsumeEffect.Type<?>> CONSUME_EFFECT_TYPE = registerSimple(Registries.CONSUME_EFFECT_TYPE, (registry) -> {
    ...
    public static final Registry<TicketType> TICKET_TYPE = registerSimple(Registries.TICKET_TYPE, (registry) -> {
    ...
    public static final Registry<OutgoingRpcMethod<?, ?>> OUTGOING_RPC_METHOD = registerSimple(Registries.OUTGOING_RPC_METHOD, (registry) -> {
        return OutgoingRpcMethods.SERVER_STARTED;
    });
    public static final Registry<MapCodec<? extends TestEnvironmentDefinition>> TEST_ENVIRONMENT_DEFINITION_TYPE = registerSimple(Registries.TEST_ENVIRONMENT_DEFINITION_TYPE, TestEnvironmentDefinition::bootstrap);
    public static final Registry<MapCodec<? extends GameTestInstance>> TEST_INSTANCE_TYPE = registerSimple(Registries.TEST_INSTANCE_TYPE, GameTestInstance::bootstrap);
    public static final Registry<MapCodec<? extends SpawnCondition>> SPAWN_CONDITION_TYPE = registerSimple(Registries.SPAWN_CONDITION_TYPE, SpawnConditions::bootstrap);
    ...
    public static final Registry<MapCodec<? extends InputControl>> INPUT_CONTROL_TYPE = registerSimple(Registries.INPUT_CONTROL_TYPE, InputControlTypes::bootstrap);
    ...
    public static final Registry<MapCodec<? extends PermissionCheck>> PERMISSION_CHECK_TYPE = registerSimple(Registries.PERMISSION_CHECK_TYPE, PermissionCheckTypes::bootstrap);
    public static final Registry<EnvironmentAttribute<?>> ENVIRONMENT_ATTRIBUTE = registerSimple(Registries.ENVIRONMENT_ATTRIBUTE, EnvironmentAttributes::bootstrap);
    ...
    public static final Registry<? extends Registry<?>> REGISTRY = BuiltInRegistries.WRITABLE_REGISTRY;
    ...
    private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> name, String defaultKey, BuiltInRegistries.RegistryBootstrap<T> loader) {
        return (DefaultedRegistry) internalRegister(name, new DefaultedMappedRegistry(defaultKey, name, Lifecycle.stable(), false), loader);
    }

    private static <T> DefaultedRegistry<T> registerDefaultedWithIntrusiveHolders(ResourceKey<? extends Registry<T>> name, String defaultKey, BuiltInRegistries.RegistryBootstrap<T> loader) {
        return (DefaultedRegistry) internalRegister(name, new DefaultedMappedRegistry(defaultKey, name, Lifecycle.stable(), true), loader);
    }

    private static <T, R extends WritableRegistry<T>> R internalRegister(ResourceKey<? extends Registry<T>> name, R registry, BuiltInRegistries.RegistryBootstrap<T> loader) {
        Bootstrap.checkBootstrapCalled(() -> {
            return "registry " + String.valueOf(name.identifier());
        });
        Identifier identifier = name.identifier();

        BuiltInRegistries.LOADERS.put(identifier, (Supplier) () -> {
            return loader.run(registry);
        });
        BuiltInRegistries.WRITABLE_REGISTRY.register(name, registry, RegistrationInfo.BUILT_IN);
        return registry;
    }
    ...
    private static void createContents() {
        BuiltInRegistries.LOADERS.forEach((identifier, supplier) -> {
            if (supplier.get() == null) {
                BuiltInRegistries.LOGGER.error("Unable to bootstrap registry '{}'", identifier);
            }
    ...
    private static <T extends Registry<?>> void validate(Registry<T> registry) {
        registry.forEach((registry1) -> {
            if (registry1.keySet().isEmpty()) {
                Identifier identifier = registry.getKey(registry1);

                Util.logAndPauseIfInIde("Registry '" + String.valueOf(identifier) + "' was empty after loading");
            }

            if (registry1 instanceof DefaultedRegistry) {
                Identifier identifier1 = ((DefaultedRegistry) registry1).getDefaultKey();

                Objects.requireNonNull(registry1.getValue(identifier1), "Missing default of DefaultedMappedRegistry: " + String.valueOf(identifier1));
            }
