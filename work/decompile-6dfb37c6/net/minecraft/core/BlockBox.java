...
public record BlockBox(BlockPos min, BlockPos max) implements Iterable<BlockPos> {
    ...
    public BlockBox extend(Direction direction, int amount) {
        return amount == 0 ? this : (direction.getAxisDirection() == Direction.AxisDirection.POSITIVE ? of(this.min, BlockPos.max(this.min, this.max.relative(direction, amount))) : of(BlockPos.min(this.min.relative(direction, amount), this.max), this.max));
    }
